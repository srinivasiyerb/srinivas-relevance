 ...
 */
@Deprecated
// not to be used after 6.3, all done with businessPaths.
...
public class SubscriptionJumpInHandlerFactory implements JumpInHandlerFactory {
	...
	public JumpInReceptionist createJumpInHandler(final UserRequest ureq) {
		...
		final String constSID = ureq.getParameter(CONST_SID);
		...
		final Subscriber sub = NotificationsManager.getInstance().getSubscriber(subscriptionKey);
		if (sub == null) { // the subscription does not exist anymore
			return null;
		}
		final Identity owner = sub.getIdentity();
		final Identity requestor = ureq.getIdentity();
		if (!owner.equalsByPersistableKey(requestor)) { throw new OLATSecurityException("tried to access subscription sub, key:" + sub.getKey() + ", owner is :"
				+ owner.getName() + ", but was requested by " + requestor.getName()); }
		...
		return new JumpInReceptionist() {
			...
			@Override
			public JumpInResult createJumpInResult(final UserRequest aureq, final WindowControl wControl) {
				...
				final OLATResourceable _ores = OresHelper.createOLATResourceableInstance(resName, resId);
				final Controller cont = ControllerFactory.createLaunchController(_ores, subidentifier, aureq, wControl, false);

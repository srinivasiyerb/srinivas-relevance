	...
	private static String checkForCharset(String hdr) {
		...
		if (charsetPos != -1) { // found!
			...
			if (endofmetastop != -1) {
				...
				if (endofcs == -1 || endofcs > endofmetastop) endofcs = hdr.indexOf('\'', charsetPos + CHARSET.length());
				if (endofcs != -1 && endofcs < endofmetastop) return hdr.substring(charsetPos + CHARSET.length(), endofcs).trim();
			}
		}
		return null;
	}

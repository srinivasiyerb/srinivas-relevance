...
public class ThreadLocalAwareLogger extends Logger {
	 ...
	 * @param threadLocalLogLevel the ThreadLocal to use to fetch overwritten log levels
	 * @param logMessageModifier optional, allows to modify log messages when they are affected by a ThreadLocal log level overwrite - NOTE: This will convert the message
	 *            passed to the
	 */
	ThreadLocalAwareLogger(String name, ThreadLocal<LogConfig> threadLocalLogLevel, LogMessageModifier logMessageModifier) {
		super(name);
		if (threadLocalLogLevel == null) { throw new IllegalArgumentException("threadLocalLogLevel must not be null"); }
	...
	private final boolean isLoggingAnyway(Priority priority) {
		if (repository.isDisabled(priority.toInt())) return false;
	 ...
	 * 
	 * @param level the level which should be checked if that is now forced to be logged by the 'threadLocalLogLevel_'
	 ...
	 * true) with a prefix.
	 * 
	 * @param originalMessage the original message which should be modified if there is a logMessageModifier

...
abstract public class AbstractExtension extends AbstractConfigOnOff implements ConfigOnOff {
	...
	public ActionExtensionSecurityCallback getActionExtensionSecurityCallback() {
		...
		if (secCallbackName != null) {
			...
			try {
				...
				if (re != null) { throw new AssertException("Could not create ActionExtensionSecurityCallback via reflection. classname: " + secCallbackName, re); }
			}
		} else {
	 ...
	 * [ Spring ]
	 * 
	 * @param nodeIdentifierIfParent The nodeIdentifierIfParent to set. if this is a parent node it should have a nodeIdentifierIfParent
	 */
	 ...
	 * [ Spring ]
	 * 
	 * @param parentTreeNodeIdentifier The parentTreeNodeIdentifier to set. set the corresponding parentNodeIdentifier, this actionExtension will get attached to
	 */

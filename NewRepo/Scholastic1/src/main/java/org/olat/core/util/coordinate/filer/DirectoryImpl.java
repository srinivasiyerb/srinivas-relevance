...
public class DirectoryImpl implements Directory {
	...
	/**
	 * true if the associated directory already existed on the filesystem before a method call on this DirectoryImpl created it. if true, then all file operations will
	 * take effect immediately, that is, the operations cannot take part in a transaction. The idea behind is that only newly created directories can be committed or
	 * rollbacked since only those are exclusively owned by this DirectoryImpl.
	 */
	private boolean realDirExisted;

	/**
	 * true if !realDirExisted and later call to getFileSystemDirectory() of this directory or any child directory caused the directory to be created (although it is a
	 * temporary directory in the same folder as the "real" one. the temp dir will be changed (renamed) to the "real" one at commit time.
	 */
	...
	/**
	 * creates the filesystem directory associated with this Directory object. the fs directory has the following structure.<br>
	 * . this directory<br>
	 * ./u the directory made available to the user (= the user calling getFileSystemDirectory())<br>
	 * ./dX, one per created child (after commit), with each X being a String made of [a-z0-9_] which represents the olatresourceable of the child Directory. ./tX, one
	 * per created child (while the transaction is in progress), with each X being a String which is unique at least within this directory and contains<br>
	 ...
	 * The temp files will be cleared upon rollback and/or on startup of the jvm (to delete those files that were left during a transaction with a jvm crash). the
	 * clearing process may even run asynchronously in order to not prolong the startup time - in order to not delete newly created temp files, the dir.lastModified

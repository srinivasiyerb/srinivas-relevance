...
public class BaseLoggingAction implements ILoggingAction {
	 ...
	 * 
	 * @param resourceAdminAction whether or not this logging action is for the eyes of an admin only (true) or also for users (false)
	...
	protected BaseLoggingAction(ActionType resourceActionType, CrudAction action, ActionVerb actionVerb, String actionObject) {
		...
		action_ = action;
		if (actionVerb == null) { throw new IllegalArgumentException("actionVerb must not be null"); }
	...
	protected BaseLoggingAction setTypeList(ResourceableTypeList typeList) {
		...
		} else if (typeList == null) { throw new IllegalArgumentException("typeList must not be null"); }
		typeList_ = typeList;
		return this;
	}
	 ...
	 * 
	 * @param javaId the java field id - or anything else which helps in your DEBUGging case - this field id will be issued when there are problems with this
	 *            ILoggingAction's ResourceableTypeList later
	 **/

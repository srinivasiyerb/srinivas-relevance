...
/**
 * ImageInfo.java Version 1.4 A Java class to determine image width, height and color depth for a number of image file formats. Written by Marco Schmidt
 * <http://www.geocities.com/marcoschmidt.geo/contact.html>. Contributed to the Public Domain. Last modification 2003-07-28 Get file format, image resolution, number of
 * bits per pixel and optionally number of images, comments and physical resolution from JPEG, GIF, BMP, PCX, PNG, IFF, RAS, PBM, PGM, PPM, PSD and SWF files (or input
 * streams).
 * <p>
 ...
 * Known limitations:
 * <ul>
 * <li>When the determination of the number of images is turned off, GIF bits per pixel are only read from the global header. For some GIFs, local palettes change this to
 * a typically larger value. To be certain to get the correct color depth, call setDetermineImageNumber(true) before calling check(). The complete scan over the GIF file
 * will take additional time.</li>
 ...
 * <li><strong>2002-03-15 </strong> Added support to recognize number of images in file. Only works with GIF. Use {@link #setDetermineImageNumber}with <code>true</code>
 * as argument to identify animated GIFs ( {@link #getNumberOfImages()}will return a value larger than <code>1</code>).</li>
 * <li><strong>2002-04-10 </strong> Fixed a bug in the feature 'determine number of images in animated GIF' introduced with version 1.1. Thanks to Marcelo P. Lima for
 * sending in the bug report. Released as 1.1.1.</li>
 * <li><strong>2002-04-18 </strong> Added {@link #setCollectComments(boolean)}. That new method lets the user specify whether textual comments are to be stored in an
 * internal list when encountered in an input image file / stream. Added two methods to return the physical width and height of the image in dpi:
 ...
 * <li><strong>2002-06-17 </strong> Added support for SWF, sent in by Michael Aird. Changed checkJpeg() so that other APP markers than APP0 will not lead to a failure
 * anymore. Released as 1.3.</li>
 * <li><strong>2003-07-28 </strong> Bug fix - skip method now takes return values into consideration. Less bytes than necessary may have been skipped, leading to flaws in
 * the retrieved information in some cases. Thanks to Bernard Bernstein for pointing that out. Released as 1.4.</li>
 * </ul>
 */
public class ImageInfo {
	/**
	 * Return value of {@link #getFormat()}for JPEG streams. ImageInfo can extract physical resolution and comments from JPEGs (only from APP0 headers). Only one image
	 * can be stored in a file.
	 */
	public static final int FORMAT_JPEG = 0;

	/**
	 * Return value of {@link #getFormat()}for GIF streams. ImageInfo can extract comments from GIFs and count the number of images (GIFs with more than one image are
	 * animations). If you know of a place where GIFs store the physical resolution of an image, please <a
	 * href="http://www.geocities.com/marcoschmidt.geo/contact.html">send me a mail </a>!
	 */
	public static final int FORMAT_GIF = 1;

	/**
	 * Return value of {@link #getFormat()}for PNG streams. PNG only supports one image per file. Both physical resolution and comments can be stored with PNG, but
	 * ImageInfo is currently not able to extract those.
	 */
	public static final int FORMAT_PNG = 2;

	/**
	 * Return value of {@link #getFormat()}for BMP streams. BMP only supports one image per file. BMP does not allow for comments. The physical resolution can be stored.
	...
	/**
	 * Return value of {@link #getFormat()}for PCX streams. PCX does not allow for comments or more than one image per file. However, the physical resolution can be
	 * stored.
	 */
	...
	/**
	 * Return value of {@link #getFormat()}for RAS streams. Sun Raster allows for one image per file only and is not able to store physical resolution or comments.
	 */
	...
	/**
	 * Call this method after you have provided an input stream or file using {@link #setInput(InputStream)}or {@link #setInput(DataInput)}. If true is returned, the file
	 * format was known and information on the file's content can be retrieved using the various getXyz methods.
	 * 
	 ...
	 * 
	 * @param index
	 * @return @throws IllegalArgumentException if index is smaller than 0 or larger than or equal to the number of comments retrieved
	 * @see #getNumberOfComments
	 */
	public String getComment(int index) {
		if (comments == null || index < 0 || index >= comments.size()) { throw new IllegalArgumentException("Not a valid comment index: " + index); }
		return (String) comments.elementAt(index);
	}

	/**
	 * If {@link #check()}was successful, returns the image format as one of the FORMAT_xyz constants from this class. Use {@link #getFormatName()}to get a textual
	...
	/**
	 * If {@link #check()}was successful and {@link #setCollectComments(boolean)} was called with <code>true</code> as argument, returns the number of comments retrieved
	 * from the input image stream / file. Any number &gt;= 0 and smaller than this number of comments is then a valid argument for the {@link #getComment(int)}method.
	 * 
	...
	/**
	 * Returns the number of images in the examined file. Assumes that <code>setDetermineImageNumber(true);</code> was called before a successful call to {@link #check()}
	...
	/**
	 * Returns the physical width of an image in inches, or <code>-1.0f</code> if width information is not available. Assumes that {@link #check}has been called
	 * successfully.
	 * 
	...
	/**
	 * To use this class as a command line application, give it either some file names as parameters (information on them will be printed to standard output, one line per
	 * file) or call it with no parameters. It will then check data given to it via standard input.
	 * 
	...
	private static void printCompact(String sourceName, ImageInfo imageInfo) {
		System.out.println(sourceName + ";" + imageInfo.getFormatName() + ";" + imageInfo.getMimeType() + ";" + imageInfo.getWidth() + ";" + imageInfo.getHeight() + ";"
				+ imageInfo.getBitsPerPixel() + ";" + imageInfo.getNumberOfImages() + ";" + imageInfo.getPhysicalWidthDpi() + ";" + imageInfo.getPhysicalHeightDpi()
				+ ";" + imageInfo.getPhysicalWidthInch() + ";" + imageInfo.getPhysicalHeightInch());
	}
	...
	/**
	 * Specify whether textual comments are supposed to be extracted from input. Default is <code>false</code>. If enabled, comments will be added to an internal list.
	 * 
	...
	/**
	 * Specify whether the number of images in a file is to be determined - default is <code>false</code>. This is a special option because some file formats require
	 * running over the entire file to find out the number of images, a rather time-consuming task. Not all file formats support more than one image. If this method is
	 * called with <code>true</code> as argument, the actual number of images can be queried via {@link #getNumberOfImages()}after a successful call to {@link #check()}.
	 * 

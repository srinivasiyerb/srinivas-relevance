...
public class ListRenderer {
	...
	public String render(FolderComponent fc, URLBuilder ubu, Translator translator, boolean iframePostEnabled) {
		...
		if (currentContainerPath.length() > 0 && currentContainerPath.charAt(0) == '/') currentContainerPath = currentContainerPath.substring(1);
		bgFlag = true;
		sb.append("<tbody>");
	...
	private void appendRenderedFile(FolderComponent fc, VFSItem child, String currentContainerPath, StringOutput sb, URLBuilder ubu, Translator translator,
		...
		if (pathAndName.length() > 0 && !pathAndName.endsWith("/")) pathAndName = pathAndName + "/";
		...
		if (canContainerVersion) {
			if (canVersion) sb.append(versions.getRevisionNr());
			sb.append("</td><td>");
		}

		// last modified
		long lastModified = child.getLastModified();
		if (lastModified != VFSConstants.UNDEFINED) sb.append(DateFormat.getDateTimeInstance(DateFormat.SHORT, DateFormat.SHORT, translator.getLocale()).format(

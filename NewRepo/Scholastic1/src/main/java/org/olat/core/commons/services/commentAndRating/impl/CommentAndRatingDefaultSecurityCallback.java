...
public class CommentAndRatingDefaultSecurityCallback implements CommentAndRatingSecurityCallback {
	 ...
	 * @param isModerator true: this user has administrative capabilities; false: this is a normal user
	 * @param isAnonymous true: user is unknown, e.g. a guest; false: user is known
	 */
	...
	public boolean canReplyToComment(UserComment comment) {
		...
		if (comment.getCreator().equalsByPersistableKey(identity)) { return false; }
		return true;
	}
	...
	/**
	 * @see org.olat.core.commons.services.commentAndRating.CommentAndRatingSecurityCallback#canUpdateComment(org.olat.core.commons.services.commentAndRating.model.UserComment,
	 *      java.util.List)
	 */
	@Override
	public boolean canUpdateComment(UserComment comment, List<UserComment> allComments) {
		...
		for (UserComment replyComment : allComments) {
			if (replyComment.getParent() != null && replyComment.getParent().getKey().equals(comment.getKey())) { return false; }
		}
		return true;
	}

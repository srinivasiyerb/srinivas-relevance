...
public class JSAndCSSComponent extends Component {
	 ...
	 * @param baseClass the class of the controller (or from the package's Manager) from where the resources will be fetched: e.g. org/olat/demo/_static/js or /css
	 *            respectively.
	 * @param jsFileNames maybe <code>null</code> An array of the jsFilenames, one entry is e.g. "script.js", which will then be resolved to
	 *            baseClass/_static/js/script.js
	 ...
	 * @param forceCssRemove if true, the given css will be removed if no longer necessary (e.g. for a custom css as in the course), otherwise it will stay in the
	 *            html-header for the whole usersession
	 * @param rawHeader -only use if neither jsFileNames nor cssFileName can be used (only for dynamically created jslibs for example)-a string (e.g. "<script...." or
	 *            "<link rel=... those entries will be refreshed after each page, that is entries causes an full page reload in ajax mode
	 * @param refreshIntervall the time in miliseconds after which (in ajax mode) a refresh of the screen is needed. -1 means infinity/no refresh. use small times with
	 *            caution, since it generates server load. after the given time, a poll (comet, push, hanging get for future release) is issued to the server to collect
	 *            the dirty components and rerender them. when more than one interval is requested, then the minimum is taken.
	 */
	...
	/**
	 * @param refreshIntervall - refresh intervall in ms, -1 no more polling i.e. <=1000 no more polling according to
	 *            /olatcore/org/olat/core/gui/control/winmgr/_content/serverpart.html
	 */

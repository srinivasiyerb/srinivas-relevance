...
public class SyncHelper {
	...
	public static Persistable findInListByKey(List persistables, Persistable persistable) {
		...
		for (Iterator iter = persistables.iterator(); iter.hasNext();) {
			...
			if (ppit.getKey().equals(key)) { return ppit; }
		}
		return null;
	}
	...
	/*
	 * public static List replacePersistable(List listOfPersitables, Persistable newPersistable) { for (Iterator iter = listOfPersitables.iterator(); iter.hasNext();) {
	 * Persistable entry = (Persistable) iter.next(); if (entry.getKey().equals(newPersistable.getKey())) { int index = listOfPersitables.indexOf(entry);
	 * listOfPersitables.add(index, newPersistable); } } return listOfPersitables; }
	 */
}

...
public class ClasspathMediaResource extends LogDelegator implements MediaResource {
	...
	private void init(String packageName) {
		if (url != null) { // null in case of resources not found.
				...
				if (WebappHelper.getContextRoot().startsWith("/")) {
					...
					if (size == null || lastModified == null) {
						...
						try {
							...
							if (entry == null) {
								logWarn("jar resource at location '" + location + "' and package " + packageName + " was not found, could not resolve entry relPath::"
										+ relPath, null);
							} else {
			...
			if (isLogDebugEnabled()) {
				logDebug("resource found at URL::" + this.url + " for package::" + packageName + " and location::" + location + ", filesize::" + size + " lastModified::"
						+ lastModified, null);
			}
		} else {
	...
	public InputStream getInputStream() {
		...
		try {
			...
			logWarn("cannot get inputstream for url:" + url.toExternalForm()

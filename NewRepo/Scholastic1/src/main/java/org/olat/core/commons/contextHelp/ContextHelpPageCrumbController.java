...
class ContextHelpPageCrumbController extends CrumbBasicController {
	...
	private void initPageResourceMapper() {
		...
		Mapper pageResourceMapper = new Mapper() {
			...
			public MediaResource handle(String relPath, HttpServletRequest request) {
				...
				if (!mr.resourceExists()) { return new NotFoundMediaResource(relPath); }
				return mr;
			}
		};
	...
	private void addPageRating(UserRequest ureq) {
		if (!ContextHelpModule.isContextRatingEnabled() || !ureq.getUserSession().isAuthenticated() || contextHelpPageVC == null) return;

...
public class ImageHelper {
	...
	public static Size scaleImage(InputStream image, VFSLeaf scaledImage, int maxWidth, int maxHeight) {
		...
		try {
			...
			if (writeTo(scaleTo(imageSrc, scaledSize), bos, scaledSize, getImageFormat(scaledImage))) { return scaledSize; }
			return null;
		} catch (IOException e) {
			return null;
		} finally {
	...
	public static Size scaleImage(VFSLeaf image, VFSLeaf scaledImage, int maxWidth, int maxHeight) {
		...
		try {
			...
			if (!scaledSize.isChanged() && isSameFormat(image, scaledImage)) {
				...
				return scaledSize;
			} else if (writeTo(scaleTo(imageSrc, scaledSize), bos, scaledSize, getImageFormat(scaledImage))) { return scaledSize; }
			return null;
		} catch (IOException e) {
			return null;
		} finally {
	...
	public static Size scaleImage(BufferedImage image, VFSLeaf scaledImage, int maxWidth, int maxHeight) {
		...
		try {
			...
			if (writeTo(scaleTo(image, scaledSize), bos, scaledSize, getImageFormat(scaledImage))) { return scaledSize; }
			return null;
		} catch (Exception e) {
			return null;
		} finally {
	...
	public static boolean scaleImage(File image, String imageExt, File scaledImage, int maxWidth, int maxHeight) {
		try {
			...
			Size scaledSize = calcScaledSize(imageSrc, maxWidth, maxHeight);
			if (!scaledSize.isChanged() && isSameFormat(image, imageExt, scaledImage)) { return FileUtils.copyFileToFile(image, scaledImage, false); }
	...
	private static String getImageFormat(File image) {
		...
		if (StringHelper.containsNonWhitespace(extension)) { return extension.toLowerCase(); }
	...
	private static boolean isSameFormat(VFSLeaf source, VFSLeaf scaled) {
		...
		if (sourceExt != null && sourceExt.equals(scaledExt)) { return true; }
		return false;
	}
	...
	private static Size calcScaledSize(BufferedImage image, int maxWidth, int maxHeight) {
		...
		if (maxHeight > height && maxWidth > width) { return new Size(width, height, false); }

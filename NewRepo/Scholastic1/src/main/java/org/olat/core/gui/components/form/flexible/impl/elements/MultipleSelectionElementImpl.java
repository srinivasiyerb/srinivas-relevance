...
public class MultipleSelectionElementImpl extends FormItemImpl implements MultipleSelectionElement {
	...
	/**
	 * @see org.olat.core.gui.components.form.flexible.elements.MultipleSelectionElement#isAtLeastSelected(int, java.lang.String)
	 */
	@Override
	...
	public void setSelectedValues(String[] values) {
		...
		for (int i = 0; i < values.length; i++) {
			...
			if (!foundKey) {
				Tracing.logWarn("submitted key '" + key + "' was not found in the keys of formelement named " + this.getName() + " , keys=" + Arrays.asList(keys),
						this.getClass());
			} else {
				selected.add(key);
			}
	...
	public static FormLayouter createVerticalLayout(String name, int columns) {
		...
		if (columns < 1 || columns > 2) throw new AssertException("Currently only 1 or 2 columns are implemented");

...
public class ClassPathStaticDispatcher extends LogDelegator implements Dispatcher {
	 ...
	 * 
	 * @param copyStaticFilesConfig true: copy static files and deliver via StaticMediaDispatcher; false: don't copy files and deliver from classpath
	 * @param dispatcherPath : path where to dispatch files ( e.g. '/classpath/')
	 */
	 ...
	 * 
	 * @param globalNameClass class for the name of the mapper. the name of the mapper is the name of the package name for this class
	 * @return the path under which this mapper will be called, without / at the end, e.g. /olat/classpath/612/org.olat.demo.tabledemo (the 612 here is the versionId to
	 *         guarantee the uniqueness across releases to trick out buggy css browser caches)
	 */
	...
	public void copyStaticClassPathFiles() {
		...
		logInfo("Copying static file from webapp source path::" + srcPath + " - be patient, this can take a while the first time when you hava jsmath files installed",
				null);

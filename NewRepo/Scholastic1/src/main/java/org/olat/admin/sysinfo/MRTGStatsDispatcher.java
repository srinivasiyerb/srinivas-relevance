...
public class MRTGStatsDispatcher implements Dispatcher {
	...
	private MRTGStatsDispatcher(final CoordinatorManager coordinatorManager) {
		...
		try {
			...
			dispatchProbe_ = codepointClient_.startProbingBetween("org.olat.core.dispatcher.DispatcherAction.execute-start",
			...
			dbQueryListProbe_ = codepointClient_.startProbingBetween("org.olat.core.commons.persistence.DBQueryImpl.list-entry",
	...
	private void returnMRTGStats(final HttpServletRequest request, final HttpServletResponse response) {
		...
		if (command.equals("users")) { // get user stats of (authenticated) usersessions
			...
			final long currentErrorCount = Tracing.getErrorCount();
			...
			final org.olat.core.commons.persistence.SimpleProbe probe = DBQueryImpl.listTableStatsMap_.get("org.olat.basesecurity.SecurityGroupMembershipImpl");
			if (probe == null) {
				result.append("0\n0\n0\n");
			} else {
			...
			final org.olat.core.commons.persistence.SimpleProbe probe = DBQueryImpl.listTableStatsMap_.get("org.olat.group.area.BGAreaImpl");
			if (probe == null) {
				result.append("0\n0\n0\n");
			} else {
				result.append(roundedValueOf(probe.getSum()));
			...
			final org.olat.core.commons.persistence.SimpleProbe probe = DBQueryImpl.listTableStatsMap_.get("org.olat.group.BusinessGroupImpl");
			if (probe == null) {
				result.append("0\n0\n0\n");
			} else {
				result.append(roundedValueOf(probe.getSum()));
			...
			final org.olat.core.commons.persistence.SimpleProbe probe = DBQueryImpl.listTableStatsMap_.get("org.olat.resource.OLATResourceImpl");
			if (probe == null) {
				result.append("0\n0\n0\n");
			} else {
				result.append(roundedValueOf(probe.getSum()));
			...
			final org.olat.core.commons.persistence.SimpleProbe probe = DBQueryImpl.listTableStatsMap_.get("THEREST");
			if (probe == null) {
				result.append("0\n0\n0\n");
			} else {
				result.append(roundedValueOf(probe.getSum()));
			...
			final Set<Entry<String, org.olat.core.commons.persistence.SimpleProbe>> entries = DBQueryImpl.listTableStatsMap_.entrySet();
			final Set<Entry<String, org.olat.core.commons.persistence.SimpleProbe>> nonRegisteredEntries = new HashSet<Entry<String, org.olat.core.commons.persistence.SimpleProbe>>();
			long sum = 0;
			for (final Iterator<Entry<String, org.olat.core.commons.persistence.SimpleProbe>> it = entries.iterator(); it.hasNext();) {
				final Entry<String, org.olat.core.commons.persistence.SimpleProbe> entry = it.next();
				if (!DBQueryImpl.registeredTables_.contains(entry.getKey())) {
					nonRegisteredEntries.add(entry);
					sum += entry.getValue().getSum();
				}
			}
			final List<Entry<String, org.olat.core.commons.persistence.SimpleProbe>> list = new LinkedList<Entry<String, org.olat.core.commons.persistence.SimpleProbe>>(
					nonRegisteredEntries);
			Collections.sort(list, new Comparator<Entry<String, org.olat.core.commons.persistence.SimpleProbe>>() {

				public int compare(final Entry<String, org.olat.core.commons.persistence.SimpleProbe> o1,
						final Entry<String, org.olat.core.commons.persistence.SimpleProbe> o2) {
			...
			for (final Iterator<Entry<String, org.olat.core.commons.persistence.SimpleProbe>> it = list.iterator(); it.hasNext();) {
				final Entry<String, org.olat.core.commons.persistence.SimpleProbe> entry = it.next();
				Tracing.logInfo(
						"MRTGStats: table '" + entry.getKey() + "' uses up " + entry.getValue().getSum() + "ms of a total of " + sum + "ms, which is "
								+ Math.round(1000.0 * entry.getValue().getSum() / sum) / 10 + "%", getClass());
				entry.getValue().reset();
			}
		} else if (command.equals("LifeCycleEntry")) { // LifeCycleEntry
			final org.olat.core.commons.persistence.SimpleProbe probe = DBQueryImpl.listTableStatsMap_.get("org.olat.commons.lifecycle.LifeCycleEntry");
			if (probe == null) {
				result.append("0\n0\n0\n");
			} else {
				result.append(roundedValueOf(probe.getSum()));

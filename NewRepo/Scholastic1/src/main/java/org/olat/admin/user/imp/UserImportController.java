...
public class UserImportController extends BasicController {
	 ...
	 * @param ureq
	 * @param wControl
	 * @param canCreateOLATPassword true: workflow offers column to create passwords; false: workflow does not offer pwd column
	 */
	...
	private Identity doCreateAndPersistIdentity(final List<String> singleUser) {
		...
		final List<UserPropertyHandler> userProperties = userPropertyHandlers;
		int col = 4;
		...
		final Identity ident = AuthHelper.createAndPersistIdentityAndUserWithUserGroup(login, pwd, newUser);
		return ident;
	}

				...
				if (!line.equals("")) { // ignore empty lines
					...
					if (ident != null) {
						...
						for (int j = 0; j < userPropertyHandlers.size(); j++) {
							...
							final boolean isMandatoryField = um.isMandatoryUserProperty(usageIdentifyer, userPropertyHandler);
							...
							if ((thisKey.equals(UserConstants.INSTITUTIONALEMAIL) || thisKey.equals(UserConstants.EMAIL)) && !thisValue.isEmpty()) {
								...
								final Identity identity = UserManager.getInstance().findIdentityByEmail(thisValue);
								if (identity != null) {

...
public class Authentication {
	...
	public Response login(@PathParam("username") final String username, @QueryParam("password") final String password, @Context final HttpServletRequest httpRequest) {
		...
		final Identity identity = OLATAuthenticationController.authenticate(username, password);
		if (identity == null) { return Response.serverError().status(Status.UNAUTHORIZED).build(); }

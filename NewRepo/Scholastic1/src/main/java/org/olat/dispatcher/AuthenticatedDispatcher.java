...
public class AuthenticatedDispatcher implements Dispatcher {
	...
	public AuthenticatedDispatcher(final boolean forceSecureAccessOnly) {
				...
				synchronized (dts) { // o_clusterok per:fj user session
					...
					if (dt == null) {
						...
						try {
							...
							final Class site = ores.getClass().getClassLoader().loadClass(ores.getResourceableTypeName());
						...
						if (isSiteInstance) {
							...
							final JumpInResult jres = jh.createJumpInResult(ureq, cc.getWindowControl());
							...
							final HttpServletRequest hsr = ureq.getHttpReq();

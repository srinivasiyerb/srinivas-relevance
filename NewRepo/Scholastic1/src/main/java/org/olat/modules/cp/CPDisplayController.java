...
public class CPDisplayController extends BasicController {
	...
	CPDisplayController(final UserRequest ureq, final WindowControl wControl, final VFSContainer rootContainer, final boolean showMenu, final boolean activateFirstPage,
		...
		if (mani == null || !(mani instanceof VFSLeaf)) { throw new OLATRuntimeException("error.manifest.missing", null, this.getClass().getPackage().getName(), "CP "
				+ rootContainer + " has no imsmanifest", null); }
		...
		if (activateFirstPage) {
			...
			if (node == null) { throw new OLATRuntimeException(CPDisplayController.class, "root node of content packaging was null, file:" + rootContainer, null); }
	...
	public void switchToPage(final UserRequest ureq, final TreeEvent te) {
		...
		// security check
		if (identifierRes.indexOf("../") != -1) { throw new AssertException("a non-normalized url encountered in a manifest item:" + identifierRes); }
	...
	public String getNodeByUri(final String uri) {
		if (StringHelper.containsNonWhitespace(uri)) {
			...
			if (node != null) { return node.getIdent(); }
		}

...
public class ScormAPIandDisplayController extends MainLayoutBasicController {
	 ...
	 */
	ScormAPIandDisplayController(final UserRequest ureq, final WindowControl wControl, final boolean showMenu, final ScormAPICallback apiCallback, final File cpRoot,
			final String resourceId, final String courseIdNodeId, final String lesson_mode, final String credit_mode, final boolean previewMode, final boolean activate) {
		super(ureq, wControl);
		...
		scormAdapter.init(cpRoot, resourceId, courseIdNodeId, FolderConfig.getCanonicalRoot(), this.username,
				ureq.getIdentity().getUser().getProperty(UserConstants.LASTNAME, loc) + ", " + ureq.getIdentity().getUser().getProperty(UserConstants.FIRSTNAME, loc),
				lesson_mode, credit_mode, this.hashCode());
		...
		final File mani = new File(cpRoot, "imsmanifest.xml");
		if (!mani.exists()) { throw new OLATRuntimeException("error.manifest.missing", null, getClass().getName(), "CP " + cpRoot.getAbsolutePath()
	...
	private void cleanUpCollectedScoData() {
		if (scorm_lesson_mode.equals(ScormConstants.SCORM_MODE_BROWSE) || scorm_lesson_mode.equals(ScormConstants.SCORM_MODE_REVIEW)) {
			final String path = FolderConfig.getCanonicalRoot() + "/tmp/" + this.hashCode();

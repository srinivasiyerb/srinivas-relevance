...
public class EPMultipleMapController extends BasicController implements Activateable {
	...
	private void initOrUpdateMaps(final UserRequest ureq) {
		...
		final List<String> artAmount = new ArrayList<String>(userMaps.size());
		final List<Integer> childAmount = new ArrayList<Integer>(userMaps.size());
		final List<String> mapStyles = new ArrayList<String>(userMaps.size());
		final List<Date> deadLines = new ArrayList<Date>(userMaps.size());
		final List<String> owners = new ArrayList<String>(userMaps.size());
		final List<String> amounts = new ArrayList<String>(userMaps.size());
		...
		for (final PortfolioStructure portfolioStructure : allUsersStruct) {
			if (portfolioStructure.getRoot() == null) { // only show maps
				...
				if (map instanceof EPStructuredMap) {
					...
					final RepositoryEntry repoEntry = RepositoryManager.getInstance().lookupRepositoryEntry(resource.getOLATResourceable(), false);
					if (repoEntry != null) {
				...
				if (!secCallback.isOwner()) {
					final List<Identity> ownerIdents = BaseSecurityManager.getInstance().getIdentitiesOfSecurityGroup(map.getOwnerGroup());
					final List<String> identNames = new ArrayList<String>();
					for (final Identity identity : ownerIdents) {
						final String fullName = identity.getUser().getProperty(UserConstants.FIRSTNAME, null) + " "
								+ identity.getUser().getProperty(UserConstants.LASTNAME, null);
	...
	protected void event(final UserRequest ureq, final Component source, final Event event) {
		if (source instanceof Link) {
			...
			final PortfolioStructureMap selMap = (PortfolioStructureMap) srcLink.getUserObject();

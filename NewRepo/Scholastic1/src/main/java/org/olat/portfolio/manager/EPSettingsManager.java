...
public class EPSettingsManager extends BasicManager {
	...
	public Map<String, Boolean> getArtefactAttributeConfig(final Identity ident) {
		...
		final Property p = pm.findProperty(ident, null, null, EPORTFOLIO_CATEGORY, EPORTFOLIO_ARTEFACTS_ATTRIBUTES);
	...
	public List<EPFilterSettings> getSavedFilterSettings(final Identity ident) {
		final PropertyManager pm = PropertyManager.getInstance();
		final Property p = pm.findProperty(ident, null, null, EPORTFOLIO_CATEGORY, EPORTFOLIO_FILTER_SETTINGS);
	...
	public String getUsersPreferedArtefactViewMode(final Identity ident, final String context) {
		final PropertyManager pm = PropertyManager.getInstance();
		final Property p = pm.findProperty(ident, null, null, EPORTFOLIO_CATEGORY, EPORTFOLIO_ARTEFACTS_VIEWMODE + "." + context);
		if (p != null) {
	...
	public Long getUsersLastUsedPortfolioStructureKey(final Identity ident) {
		final PropertyManager pm = PropertyManager.getInstance();
		final Property p = pm.findProperty(ident, null, null, EPORTFOLIO_CATEGORY, EPORTFOLIO_LASTUSED_STRUCTURE);
		if (p != null) { return p.getLongValue(); }
		return null;
	}

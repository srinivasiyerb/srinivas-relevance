...
public class ControllerFactory {
	 ...
	 * @param ureq
	 * @param wControl
	 * @param exceptIfNoneFound if true, then an exception will be thrown if no controller could be created for whatever reason. if false, the call will just return null.
	...
	public static MainLayoutController createLaunchController(final OLATResourceable olatResourceable, final String initialViewIdentifier, final UserRequest ureq,
		...
		if (olatResourceable == null) {
			...
			final BusinessGroup bg = bgm.loadBusinessGroup(olatResourceable.getResourceableId(), exceptIfNoneFound);
			...
			final OLATResourceable fakedCourseResource = OresHelper.createOLATResourceableInstance(CourseModule.class, olatResourceable.getResourceableId());
			final RepositoryEntry re = rm.lookupRepositoryEntry(fakedCourseResource, false);
			if (re == null) { return null;// found no repositoryEntry => return null
			}
		...
		// --- home ---
			...
			if (!roles.isGroupManager()) { throw new OLATSecurityException("Tried to launch a BGContextManagementController, but is not in group groupmanager " + roles); }
			...
			if (!roles.isOLATAdmin()) { throw new OLATSecurityException("Tried to launch a GUIDemoMainController, but is not in admin group " + roles); }
	...
	public static String translateResourceableTypeName(final String resourceableTypeName, final Locale locale) {
		...
		final Translator trans = Util.createPackageTranslator(BaseChiefController.class, locale);

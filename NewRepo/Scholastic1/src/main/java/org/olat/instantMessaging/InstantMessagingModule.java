...
public class InstantMessagingModule implements Initializable, Destroyable, UserDataDeletable, GenericEventListener {
	...
	public void init() {
		if (config.isEnabled()) {
			...
			final List props = propertyManager.findProperties(null, null, null, "classConfig", createPropertyName(this.getClass(), CONFIG_SYNCED_BUDDY_GROUPS));
			if (props.size() == 0) {

				if (config.isSyncPersonalGroups()) {
					instantMessaging.synchronizeAllBuddyGroupsWithIMServer();
				}
				final Property property = propertyManager.createPropertyInstance(null, null, null, "classConfig",
						createPropertyName(this.getClass(), CONFIG_SYNCED_BUDDY_GROUPS), null, null, Boolean.toString(true), null);
	 ...
	 * @return String
	 */
	private String createPropertyName(final Class clazz, final String configurationName) {
	...
	private void checkAndCreateTestUsers() {
		...
		if (auth == null) { // create new authentication for provider
			BaseSecurityManager.getInstance().createAndPersistAuthentication(identity, ClientManager.PROVIDER_INSTANT_MESSAGING, identity.getName(), "test");
		...
		auth = BaseSecurityManager.getInstance().findAuthentication(identity, ClientManager.PROVIDER_INSTANT_MESSAGING);
		if (auth == null) { // create new authentication for provider
			BaseSecurityManager.getInstance().createAndPersistAuthentication(identity, ClientManager.PROVIDER_INSTANT_MESSAGING, identity.getName(), "olat");
	...
	public void event(final Event event) {
		...
		if (event instanceof FrameworkStartedEvent) {
			...
			try {
				final List props = propertyManager.findProperties(null, null, null, "classConfig", createPropertyName(this.getClass(), CONFIG_SYNCED_LEARNING_GROUPS));
				if (props.size() == 0) {
					if (isSyncLearningGroups()) {
						instantMessaging.synchronizeLearningGroupsWithIMServer();
						final Property property = propertyManager.createPropertyInstance(null, null, null, "classConfig",
								createPropertyName(this.getClass(), CONFIG_SYNCED_LEARNING_GROUPS), null, null, Boolean.toString(true), null);
						propertyManager.saveProperty(property);
					}
				}

...
public class ClientHelper extends LogDelegator {
	...
	public List<String> getDistinctRoster() {
		if (imc.isConnected()) { return createLinkList(imc.getRoster().getEntries().iterator(), null); }
	...
	public List<String> getRosterGroupEntries(final String groupname) {
		if (imc.isConnected()) { return createLinkList(imc.getRoster().getGroup(groupname).getEntries().iterator(), groupname); }
	...
	private List<String> createLinkList(final Iterator<RosterEntry> iter, final String groupname) {
		...
		for (final Iterator<RosterEntry> i = iter; i.hasNext();) {
			...
			if (getShowOfflineBuddies() || entryPresence != "offline") {
				...
				final Identity ident = BaseSecurityManager.getInstance().findIdentityByName(entry.getName());
				if (ident != null) {
					link.setCustomDisplayText(ident.getUser().getProperty(UserConstants.FIRSTNAME, null) + " "
							+ ident.getUser().getProperty(UserConstants.LASTNAME, null) + " (" + ident.getName() + ")");
				} else {

...
public class AdminUserConnection {
	 ...
	 * @param nodeId
	 */
	private AdminUserConnection(final String serverName, final String adminUsername, final String adminPassword, final String nodeId, final boolean enabled) {
		if (serverName == null || adminUsername == null || adminPassword == null) { throw new StartupException(
				"Instant Messaging settings for admin user are not correct"); }
	...
	private void connect(final String servername, final String adminUsername, final String adminPassword, final String nodeId) {
		try {
		...
		if (connection == null || !connection.isAuthenticated()) { throw new StartupException("could not connect to instant messaging server with admin user!"); }

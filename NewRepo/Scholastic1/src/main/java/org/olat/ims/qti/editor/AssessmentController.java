...
public class AssessmentController extends TabbableDefaultController implements ControllerEventListener {
	 ...
	 * @param wControl
	 */
	public AssessmentController(final Assessment assessment, final QTIEditorPackage qtiPackage, final UserRequest ureq, final WindowControl wControl,
	...
	public void event(final UserRequest ureq, final Component source, final Event event) {
		if (source == main) {
			if (event.getCommand().equals("sao")) { // asessment options submitted
				...
				//
				if (!surveyMode && !restrictedEdit) {
					final Control tmpControl = QTIEditHelper.getControl(assessment);

...
public class MemberListWizardController extends BasicController {
	...
	private GenericObjectArrayTableDataModel getGroupOrAreaChoiceTableDataModel(final BGContext context) {
		...
		if (GROUPS_MEMBERS.equals(wizardType)) {
			final List<BusinessGroup> groups = BGContextManagerImpl.getInstance().getGroupsOfBGContext(context);
			...
			final List<BGArea> areas = BGAreaManagerImpl.getInstance().findBGAreasOfBGContext(context);
		...
		if (source == wizardController) {
			...
			if (event == Event.DONE_EVENT) {
				...
				if (selected.size() == 0) {
					...
					final File outputFile = archiveMembers(ureq);
	 ...
	 */
	private File archiveMembers(final UserRequest ureq) {
		File outputFile = null;
		final List columnList = this.getColumList();
		final List groupList = this.getGroupList();
		final String archiveType = this.getArchiveType();
		final List areaList = this.getAreaList();

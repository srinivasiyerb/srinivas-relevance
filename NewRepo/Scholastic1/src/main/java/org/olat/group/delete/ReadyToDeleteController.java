...
public class ReadyToDeleteController extends BasicController {
	...
	public ReadyToDeleteController(final UserRequest ureq, final WindowControl wControl) {
		...
		tableModelTypeTranslator = BGTranslatorFactory.createBGPackageTranslator(Util.getPackageName(ReadyToDeleteController.class), /* doesnt matter */
	...
	private void initializeTableController(final UserRequest ureq) {
		...
		readyToDeleteContent.contextPut("header", translate("ready.to.delete.header", Integer.toString(GroupDeletionManager.getInstance().getDeleteEmailDuration())));
		readyToDeletePanel.setContent(readyToDeleteContent);

	}

	protected void updateGroupList() {
		final List l = GroupDeletionManager.getInstance().getGroupsReadyToDelete(GroupDeletionManager.getInstance().getDeleteEmailDuration());

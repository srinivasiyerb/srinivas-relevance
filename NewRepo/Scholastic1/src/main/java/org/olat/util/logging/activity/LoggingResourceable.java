...
public class LoggingResourceable implements ILoggingResourceable {
	 ...
	 * @return
	 */
	private static String restrictStringLength(final String arg, final int maxBytes, final String argNameForLogging, final boolean log) {
		if (arg == null) {
		...
		try {
			...
			if (log) {
				log_.error("restrictStringLength: " + argNameForLogging + " too long. Allowed " + maxBytes + ", actual: " + arg.getBytes().length + ", value=" + arg);
			}
	...
	public static LoggingResourceable wrapScormRepositoryEntry(final RepositoryEntry scormRepoEntry) {
		if (scormRepoEntry == null) { throw new IllegalArgumentException("scormRepoEntry must not be null"); }
	...
	public static LoggingResourceable wrapWikiOres(final OLATResourceable olatResourceable) {
		if (olatResourceable == null) { throw new IllegalArgumentException("olatResourceable must not be null"); }
	...
	public static LoggingResourceable wrap(final Forum forum) {
		final String name;
		final List<Message> forumMessages = ForumManager.getInstance().getMessagesByForum(forum);
	 ...
	 */
	public static LoggingResourceable wrap(final Kalendar calendar) {
	...
	public boolean equals(final Object obj) {
		if (!(obj instanceof LoggingResourceable)) {
			...
			return true;
		} else if (hashCode() != obj.hashCode()) { return false; }

		final LoggingResourceable lri = (LoggingResourceable) obj;
		if (!type_.equals(lri.type_)) { return false; }
		if (id_ == null) {
			if (lri.id_ != null) { return false; }
		} else if (lri.id_ == null) {
			return false;
		} else if (!id_.equals(lri.id_)) { return false; }
		if (resourceableType_ != lri.resourceableType_) { return false; }
		if (resourceable_ == null && lri.resourceableType_ != null) { return false; }
		if (resourceable_ != null && lri.resourceableType_ == null) { return false; }
		if (!resourceable_.getResourceableTypeName().equals(lri.resourceable_.getResourceableTypeName())) { return false; }
		if (!resourceable_.getResourceableId().equals(lri.resourceable_.getResourceableId())) { return false; }
	...
	public boolean correspondsTo(final ContextEntry ce) {
		...
		if (ceResourceable == null) { return false; }

		if (resourceable_ != null) {
			if (ceResourceable.getResourceableTypeName().equals(resourceable_.getResourceableTypeName())
					&& ceResourceable.getResourceableId().equals(resourceable_.getResourceableId())) { return true; }
			if (ceResourceable instanceof RepositoryEntry) {
				...
				if (ores != null && ores.getResourceableTypeName().equals(resourceable_.getResourceableTypeName())
						&& ores.getResourceableId().equals(resourceable_.getResourceableId())) { return true; }
				...
				final RepositoryEntry re = RepositoryManager.getInstance().lookupRepositoryEntry(ceResourceable.getResourceableId());
				if (re != null) {
					final OLATResource ores = re.getOlatResource();
					if (ores != null && ores.getResourceableTypeName().equals(resourceable_.getResourceableTypeName())
							&& ores.getResourceableId().equals(resourceable_.getResourceableId())) { return true; }
		...
		// we still try...
		if (type_.equals(StringResourceableType.targetIdentity.name()) && ceResourceable.getResourceableTypeName() == "Identity") { return id_.equals(String
				.valueOf(ceResourceable.getResourceableId())); }
		return false;
	}

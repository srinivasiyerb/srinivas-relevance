...
public class CatalogController extends BasicController implements Activateable {
	...
	public CatalogController(final UserRequest ureq, final WindowControl wControl, final String jumpToNode) {
		...
		final List<CatalogEntry> rootNodes = cm.getRootCatalogEntries();
		CatalogEntry rootce;
	...
	public void event(final UserRequest ureq, final Component source, final Event event) {
		if (source == myContent) { // links from vc -> here a link used in a
			...
			if (command.startsWith(CATENTRY_CHILD)) { // child clicked
				...
				if (repoEntry == null) { throw new AssertException("a leaf did not have a repositoryentry! catalogEntry = key:" + cur.getKey() + ", title "
						+ cur.getName()); }
	...
	public void event(final UserRequest ureq, final Controller source, final Event event) {
		...
		if (source == catalogToolC) {
			...
			if (event.getCommand().equals(ACTION_ADD_CTLGCATEGORY)) {
				catModificationLock = CoordinatorManager.getInstance().getCoordinator().getLocker()
						.acquireLock(OresHelper.createOLATResourceableType(CatalogController.class), ureq.getIdentity(), LOCK_TOKEN);
			 ...
			 */
				...
				if (secGroup == null) {
					final CatalogEntry reloaded = cm.loadCatalogEntry(currentCatalogEntry);
			 ...
			 */
				 ...
				 */
				final BaseSecurity mngr = BaseSecurityManager.getInstance();
				...
				for (int i = historyStack.size() - 1; i >= 0 && tmpIdent.isEmpty(); i--) {
					...
					if (tmpOwn != null) {
						tmpIdent = mngr.getIdentitiesOfSecurityGroup(tmpOwn);
					} else {
						tmpIdent = emptyList;
					}
				}
			 ...
			 */
				...
				final OLATResourceable ores = cm.createOLATResouceableFor(currentCatalogEntry);
			 ...
			 */
				...
				final CatalogEntry newRoot = (CatalogEntry) cm.getRootCatalogEntries().get(0);
		 ...
		 */
			...
			if (event.getCommand().equals(RepositoryTableModel.TABLE_ACTION_SELECT_LINK)) {
				...
				/*
				 * open the confirm form, which allows to change the link-title, link-description.
				 */
		 ...
		 */
			...
			if (event instanceof IdentitiesAddEvent) { // FIXME:chg: Move into seperate RepositoryOwnerGroupController like BusinessGroupEditController ?
				...
				final List<Identity> list = identitiesAddedEvent.getAddIdentities();
				final BaseSecurity securityManager = BaseSecurityManager.getInstance();
				for (final Identity identity : list) {
				...
				final List<Identity> list = identitiesRemoveEvent.getRemovedIdentities();
				for (final Identity identity : list) {
			...
			if (event == Event.DONE_EVENT) {
				final CatalogEntry ce = cm.createCatalogEntry();
			...
			final CatalogEntry reloaded = (CatalogEntry) DBFactory.getInstance().loadObject(currentCatalogEntry);
			currentCatalogEntry = reloaded;// FIXME:pb
			if (event == Event.DONE_EVENT) {
	...
	private void updateContent(final Identity identity, final CatalogEntry ce, final int ceLevel) {
		...
		for (final Object leaf : childCe) {
			...
			final ImageComponent ic = RepositoryEntryImageController.getImageComponentForRepositoryEntry(name, entry.getRepositoryEntry());
			if (ic == null) {
		...
		if (canAddLinks && !canRemoveAllLinks) {
			final List ownedLinks = cm.filterOwnedLeafs(identity, childCe);
	...
	private void importStructure() {
		final CatalogEntry oldRoot = (CatalogEntry) cm.getRootCatalogEntries().get(0);
		final SecurityGroup rootOwners = oldRoot.getOwnerGroup();
		final BaseSecurity secMgr = BaseSecurityManager.getInstance();
		final List olatAdminIdents = secMgr.getIdentitiesOfSecurityGroup(rootOwners);
		...
		final CatalogEntry dummy = cm.createCatalogEntry();
		...
		//
		final CatalogEntry newRoot = cm.createCatalogEntry();
		...
		for (int i = 1; i < lines.length; i++) {
			...
			final CatalogEntry tmp = cm.createCatalogEntry();
	...
	private void updateToolAccessRights(final UserRequest ureq, final CatalogEntry ce, final int pos) {
		...
		final OLATResourceable catEntryOres = CatalogManager.getInstance().createOLATResouceableFor(ce);

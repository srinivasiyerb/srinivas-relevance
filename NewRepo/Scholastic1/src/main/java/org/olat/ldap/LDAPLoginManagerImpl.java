...
public class LDAPLoginManagerImpl extends LDAPLoginManager implements GenericEventListener {
	 ...
	 */
	public void setSecurityManager(final BaseSecurity securityManager) {
	...
	public LdapContext bindSystem() {
		...
		try {
			...
			logError("NamingException when trying to bind system with DN::" + LDAPLoginModule.getLdapSystemDN() + " and PW::" + LDAPLoginModule.getLdapSystemPW()
					+ " on URL::" + LDAPLoginModule.getLdapUrl(), e);
			return null;
		} catch (final Exception e) {
			logError("Exception when trying to bind system with DN::" + LDAPLoginModule.getLdapSystemDN() + " and PW::" + LDAPLoginModule.getLdapSystemPW() + " on URL::"
					+ LDAPLoginModule.getLdapUrl(), e);
			return null;
		}
	...
	public Attributes bindUser(final String uid, final String pwd, final LDAPError errors) {
		...
		if (uid == null || pwd == null) {
			if (isLogDebugEnabled()) {
				logDebug("Error when trying to bind user, missing username or password. Username::" + uid + " pwd::" + pwd);
			}
	 ...
	 * 
	 * @param olatPropertyMap Map of changed OLAT properties (OLATProperty,LDAPValue)
	...
	public void createAndPersistUser(final Attributes userAttributes) {
		...
		// Create Identity
		final Identity identity = securityManager.createAndPersistIdentityAndUser(uid, user, LDAPAuthenticationController.PROVIDER_LDAP, uid, null);
	 ...
	 * @param errors LDAPError Object if user exits but not member of LDAPSecurityGroup
	...
	public Identity findIdentyByLdapAuthentication(final String uid, final LDAPError errors) {
		...
		if (identity == null) {
			...
			if (securityManager.isIdentityInSecurityGroup(identity, ldapGroup)) {
				final Authentication ldapAuth = securityManager.findAuthentication(identity, LDAPAuthenticationController.PROVIDER_LDAP);
				if (ldapAuth == null) {
				...
				if (LDAPLoginModule.isConvertExistingLocalUsersToLDAPUsers()) {
					...
					logInfo("Found identity by LDAP username that was not yet in LDAP security group. Converted user::" + uid + " to be an LDAP managed user");
					return identity;
				} else {
			...
			while (results.hasMore()) {
				...
				while (attrs.hasMore()) {
					...
					while (vals.hasMore()) {
						...
						if (value.equals(PAGED_RESULT_CONTROL_OID)) { return true; }
					}
				}
			}
			return false;
			...
			if (LDAPLoginModule.isDeleteRemovedLDAPUsersOnSync()) {
				...
				if (olatListIdentity.isEmpty()) {
					...
					if (prozente >= LDAPLoginModule.getDeleteRemovedLDAPUsersPercentage()) {
						logInfo("LDAP batch sync: more than " + LDAPLoginModule.getDeleteRemovedLDAPUsersPercentage()
						...
						logInfo("LDAP batch sync: " + deletedUserList.size() + " users deleted" + sinceSentence);
					}
				}
			} else {
				...
				logInfo("LDAP batch sync: " + deletedUserList.size() + " users detected as to be deleted" + sinceSentence
		...
		for (final Attributes userAttrs : ldapUserList) {
			...
			if (identity != null) {
				...
				if (reqAttrs == null) {
					...
					logWarn("Error in LDAP batch sync: can't create user with username::" + user + " : missing required attributes::" + ArrayUtils.toString(reqAttrs),
							null);
				}
			} else {
	...
	public synchronized boolean acquireSyncLock() {
		if (batchSyncIsRunning) { return false; }
		batchSyncIsRunning = true;
		return true;
	}

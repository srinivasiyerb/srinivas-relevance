...
public class WebDAVAuthManager {
	...
	public static Identity authenticate(final String login, final String pass) {
		final Identity ident = BaseSecurityManager.getInstance().findIdentityByName(login);
		if (ident == null) { return null; }
		final boolean visible = BaseSecurityManager.getInstance().isIdentityVisible(login);
		if (!visible) { return null; }

		// find WEBDAV authentication provider
		final Authentication auth = BaseSecurityManager.getInstance().findAuthentication(ident, PROVIDER_WEBDAV);

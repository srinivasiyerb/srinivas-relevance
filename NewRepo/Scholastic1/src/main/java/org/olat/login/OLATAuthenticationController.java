...
public class OLATAuthenticationController extends AuthenticationController {
	...
	public static Identity authenticate(final String login, final String pass) {
		...
		final Authentication auth = BaseSecurityManager.getInstance().findAuthentication(ident, BaseSecurityModule.getDefaultAuthProviderIdentifier());

		if (auth != null && auth.getCredential().equals(Encoder.encrypt(pass))) { return ident; }

		Tracing.createLoggerFor(OLATAuthenticationController.class).audit("Error authenticating user " + login + " via provider OLAT",
				OLATAuthenticationController.class.getName());

		return null;
	}

	private static Identity findIdentInChangingEmailWorkflow(final String login) {
		...
		if (tk != null) {
			for (final TemporaryKey temporaryKey : tk) {
				...
				if (login.equals(mails.get("changedEMail"))) { return BaseSecurityManager.getInstance().findIdentityByName(mails.get("currentEMail")); }
			}
		}
		return null;
	}

 ...
 * Controller is defined in serviceconfig/org/olat/_spring/olatextconfig.xml -> "fullWebApp.AfterLoginInterceptionControllerCreator" - bean. the Controllers itself need
 * to be appended with AfterLoginInterceptorManager.addAfterLoginControllerConfig(). Initial Date: 01.10.2009 <br>
 * 
...
public class AfterLoginInterceptionController extends BasicController {
	...
	private Long getLastRunTimeForController(final String ctrlName) {
		for (final Property prop : ctrlPropList) {
			if (prop.getName().equals(ctrlName)) { return new Long(prop.getLastModified().getTime() / 1000); }
		}
		return new Long(0);
	}

	private boolean getRunStateForController(final String ctrlName) {
		for (final Property prop : ctrlPropList) {
	...
	private void saveOrUpdatePropertyForController(final UserRequest ureq, final String ctrlName) {
		for (final Property prop : ctrlPropList) {
		...
		final Property prop = pm.createPropertyInstance(ureq.getIdentity(), null, null, PROPERTY_CAT, ctrlName, null, null, "true", null);

...
public class ProfileFormController extends FormBasicController {
	 ...
	 * @param conf The homepage configuration (decides which user profile fields are visible for everyone).
	 * @param identity The identity of the user.
	 * @param isAdministrativeUser true: user is editing another users profile as user manager; false: use is editing his own profile
	 */
	...
	protected void initForm(final FormItemContainer formLayout, final Controller listener, final UserRequest ureq) {
		...
		final List<UserPropertyHandler> homepagePropertyHanders = UserManager.getInstance().getUserPropertyHandlersFor(HomePageConfig.class.getCanonicalName(),
		...
		for (final UserPropertyHandler userPropertyHandler : this.userPropertyHandlers) {
			...
			if (formItem instanceof TextElement) {
				...
				final String value = this.identity.getUser().getProperty(propertyName, getLocale());
				if (value != null) {
			...
			if (userPropertyHandler.getName().equals("email")) {
				...
				final String key = this.identity.getUser().getProperty("emchangeKey", null);
				final TemporaryKeyImpl tempKey = rm.loadTemporaryKeyByRegistrationKey(key);
				if (tempKey != null) {
	 ...
	 * 
	 * @param config The user's home page configuration (i.e. flags for publicly visible fields).
	...
	/*
	 * (non-Javadoc)
	 * @seeorg.olat.core.gui.components.form.flexible.impl.FormBasicController# validateFormLogic(org.olat.core.gui.UserRequest)
	 */
	@Override

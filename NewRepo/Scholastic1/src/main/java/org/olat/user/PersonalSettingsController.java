...
public class PersonalSettingsController extends BasicController {
	...
	public PersonalSettingsController(final UserRequest ureq, final WindowControl wControl) {
		...
		if (!mgr.isIdentityPermittedOnResourceable(ureq.getIdentity(), Constants.PERMISSION_ACCESS, OresHelper.lookupType(this.getClass()))) { throw new OLATSecurityException(
				"Insufficient permissions to access PersonalSettingsController"); }
	...
	private boolean canChangePassword() {
		...
		if (LDAPLoginModule.isLDAPEnabled() && !LDAPLoginModule.isPropagatePasswordChangedOnLdapServer()) {
			...
			final Authentication auth = BaseSecurityManager.getInstance().findAuthentication(getIdentity(), LDAPAuthenticationController.PROVIDER_LDAP);

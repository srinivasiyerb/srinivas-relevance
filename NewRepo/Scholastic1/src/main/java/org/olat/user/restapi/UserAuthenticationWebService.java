...
public class UserAuthenticationWebService {
	...
	public Response getAuthenticationTokenList(@PathParam("username") final String username, @Context final HttpServletRequest request) {
		if (!RestSecurityHelper.isUserManager(request)) { return Response.serverError().status(Status.UNAUTHORIZED).build(); }
	...
	public Response create(@PathParam("username") final String username, final AuthenticationVO authenticationVO, @Context final HttpServletRequest request) {
		...
		if (identity == null) { return Response.serverError().status(Status.NOT_FOUND).build(); }
		if (!identity.getName().equals(username)) { return Response.serverError().status(Status.NOT_FOUND).build(); }
		...
		if (authentication == null) { return Response.serverError().status(Status.NOT_ACCEPTABLE).build(); }

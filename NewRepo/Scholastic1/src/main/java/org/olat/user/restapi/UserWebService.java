...
public class UserWebService {
	...
	private Response getUserList(final String login, final String authProvider, final String authUsername, final Map<String, List<String>> params, final UriInfo uriInfo,
		...
		if (!isUserManager(httpRequest)) { return Response.serverError().status(Status.UNAUTHORIZED).build(); }
		...
		if (StringHelper.containsNonWhitespace(authProvider) && StringHelper.containsNonWhitespace(authUsername)) {
			final Authentication auth = BaseSecurityManager.getInstance().findAuthenticationByAuthusername(authUsername, authProvider);
			if (auth == null) {
			...
			if (!params.isEmpty()) {
				...
				final List<UserPropertyHandler> propertyHandlers = um.getUserPropertyHandlersFor(PROPERTY_HANDLER_IDENTIFIER, false);
			...
			identities = BaseSecurityManager.getInstance().getIdentitiesByPowerSearch(login, userProps, true, null, null, authProviders, null, null, null, null,
	...
	public Response create(final UserVO user, @Context final HttpServletRequest request) {
		if (!isUserManager(request)) { return Response.serverError().status(Status.UNAUTHORIZED).build(); }

		// Check if login is still available
		final Identity identity = BaseSecurityManager.getInstance().findIdentityByName(user.getLogin());
		if (identity != null) {
			final Locale locale = getLocale(request);
			final Translator translator = new PackageTranslator("org.olat.admin.user", locale);
			final String translation = translator.translate("new.error.loginname.choosen");
			final ErrorVO[] errorVos = new ErrorVO[] { new ErrorVO("org.olat.admin.user", "new.error.loginname.choosen", translation) };
			return Response.ok(errorVos).status(Status.NOT_ACCEPTABLE).build();
		}

		final List<ErrorVO> errors = validateUser(user, request);
		if (errors.isEmpty()) {
			final User newUser = UserManager.getInstance().createUser(user.getFirstName(), user.getLastName(), user.getEmail());
			final Identity id = AuthHelper.createAndPersistIdentityAndUserWithUserGroup(user.getLogin(), user.getPassword(), newUser);
	...
	public Response getPortrait(@PathParam("identityKey") final Long identityKey, @Context final Request request) {
		try {
			...
			if (!portrait.exists()) { return Response.serverError().status(Status.NOT_FOUND).build(); }
	...
	public Response postPortrait(@PathParam("identityKey") final Long identityKey, @FormParam("filename") final String filename,
		...
		try {
			...
			if (!isUserManager(request) && !identity.equalsByPersistableKey(authIdentity)) { return Response.serverError().status(Status.UNAUTHORIZED).build(); }
	...
	public Response deletePortrait(@PathParam("identityKey") final Long identityKey, @Context final HttpServletRequest request) {
		try {
			...
			} else if (!isUserManager(request) && !identity.equalsByPersistableKey(authIdentity)) { return Response.serverError().status(Status.UNAUTHORIZED).build(); }
	...
	public Response getUserGroupList(@PathParam("identityKey") final Long identityKey) {
		final Identity retrievedUser = BaseSecurityManager.getInstance().loadIdentityByKey(identityKey, false);
		if (retrievedUser == null) { return Response.serverError().status(Status.NOT_FOUND).build(); }
		...
		for (final String bgType : bgTypes) {
			final List<BusinessGroup> attendedGroups = bgm.findBusinessGroupsAttendedBy(bgType, retrievedUser, null);
			...
			final List<BusinessGroup> ownedGroups = bgm.findBusinessGroupsOwnedBy(bgType, retrievedUser, null);
	...
	public Response update(@PathParam("identityKey") final Long identityKey, final UserVO user, @Context final UriInfo uriInfo, @Context final HttpServletRequest request) {
		try {
			if (user == null) { return Response.serverError().status(Status.NO_CONTENT).build(); }
			if (!isUserManager(request)) { return Response.serverError().status(Status.UNAUTHORIZED).build(); }
			...
			if (retrievedIdentity == null) { return Response.serverError().status(Status.NOT_FOUND).build(); }
	...
	private boolean validateProperty(final String name, final String value, final List<UserPropertyHandler> handlers, final List<ErrorVO> errors, final UserManager um,
			final Locale locale) {
		for (final UserPropertyHandler handler : handlers) {
			if (handler.getName().equals(name)) { return validateProperty(value, handler, errors, um, locale); }
		}
		return true;
	}

	private boolean validateProperty(final UserVO user, final UserPropertyHandler userPropertyHandler, final List<ErrorVO> errors, final UserManager um,
			final Locale locale) {
	...
	private boolean validateProperty(String value, final UserPropertyHandler userPropertyHandler, final List<ErrorVO> errors, final UserManager um, final Locale locale) {
		...
		if (!StringHelper.containsNonWhitespace(value) && um.isMandatoryUserProperty(PROPERTY_HANDLER_IDENTIFIER, userPropertyHandler)) {
			final Translator translator = new PackageTranslator("org.olat.core", locale);

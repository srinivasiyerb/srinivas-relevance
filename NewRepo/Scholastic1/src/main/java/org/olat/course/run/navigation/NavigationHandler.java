...
public class NavigationHandler {
	 ...
	 * @param calledCourseNode the coursenode to jump to; if null, the root coursenode is selected
	...
	public NodeClickedRef evaluateJumpToTreeNode(final UserRequest ureq, final WindowControl wControl, final TreeModel treeModel, final TreeEvent treeEvent,
		...
		if (!(userObject instanceof NodeEvaluation)) {
			...
			if (!prevEval.isVisible()) { throw new AssertException("clicked on a node which is not visible: treenode=" + selTN.getIdent() + ", " + selTN.getTitle()); }

...
public class ScormEditController extends ActivateableTabbableDefaultController implements ControllerEventListener {
	...
	public ScormEditController(final ScormCourseNode scormNode, final UserRequest ureq, final WindowControl wControl, final ICourse course,
		...
		if (config.get(CONFIG_KEY_REPOSITORY_SOFTKEY) != null) {
			...
			// chosen cp
			final RepositoryEntry re = getScormCPReference(config, false);
	...
	public void event(final UserRequest ureq, final Component source, final Event event) {
		if (source == chooseCPButton || source == changeCPButton) { // those must be links
			...
			if (re == null) { // we cannot preview it, because the repository entry
				...
				final ScormAPIandDisplayController previewController = ScormMainManager.getInstance().createScormAPIandDisplayController(ureq, getWindowControl(),
						showMenu, null, cpRoot, null, course.getResourceableId().toString(), ScormConstants.SCORM_MODE_BROWSE, ScormConstants.SCORM_MODE_NOCREDIT, true,
						true);
	 ...
	 */
	public static RepositoryEntry getScormCPReference(final ModuleConfiguration config, final boolean strict) {
		if (config == null) { throw new AssertException("missing config in CP"); }
		final String repoSoftkey = (String) config.get(ScormEditController.CONFIG_KEY_REPOSITORY_SOFTKEY);
	 ...
	 */
	public static void removeScormCPReference(final ModuleConfiguration moduleConfiguration) {
	 ...
	 * @param moduleConfiguration
	 */
	public static void setScormCPReference(final RepositoryEntry re, final ModuleConfiguration moduleConfiguration) {

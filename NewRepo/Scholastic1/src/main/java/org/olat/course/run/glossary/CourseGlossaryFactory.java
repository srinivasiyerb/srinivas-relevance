...
public class CourseGlossaryFactory {
	 ...
	 * @param courseConfig use the glossary configuration from the given course configuration
	 */
	public static GlossaryMarkupItemController createGlossaryMarkupWrapper(final UserRequest ureq, final WindowControl wControl, final Component tmComponent,
		...
		if (courseConfig.hasGlossary()) {
			final RepositoryEntry repoEntry = RepositoryManager.getInstance().lookupRepositoryEntryBySoftkey(courseConfig.getGlossarySoftKey(), false);
			if (repoEntry == null) {
				// seems to be removed
				return null;
			}

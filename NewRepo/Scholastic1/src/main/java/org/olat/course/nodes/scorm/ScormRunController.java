...
public class ScormRunController extends BasicController implements ScormAPICallback {
	...
	public ScormRunController(final ModuleConfiguration config, final UserRequest ureq, final UserCourseEnvironment userCourseEnv, final WindowControl wControl,
		...
		if (!ScormEditController.isModuleConfigValid(config)) { throw new AssertException("scorm run controller had an invalid module config:" + config.toString()); }
	...
	private void doLaunch(final UserRequest ureq) {
		if (cpRoot == null) {
			...
			final RepositoryEntry re = ScormEditController.getScormCPReference(config, true);
		...
		if (isPreview) {
			...
			scormDispC = ScormMainManager.getInstance().createScormAPIandDisplayController(ureq, getWindowControl(), showMenu, null, cpRoot, null, courseId,
			...
			if (isAssessable) {
				scormDispC = ScormMainManager.getInstance().createScormAPIandDisplayController(ureq, getWindowControl(), showMenu, this, cpRoot, null,
				...
				scormDispC = ScormMainManager.getInstance().createScormAPIandDisplayController(ureq, getWindowControl(), showMenu, null, cpRoot, null,
						courseId + "-" + scormNode.getIdent(), ScormConstants.SCORM_MODE_NORMAL, ScormConstants.SCORM_MODE_CREDIT, false, true);
			} else {
				scormDispC = ScormMainManager.getInstance().createScormAPIandDisplayController(ureq, getWindowControl(), showMenu, null, cpRoot, null, courseId,

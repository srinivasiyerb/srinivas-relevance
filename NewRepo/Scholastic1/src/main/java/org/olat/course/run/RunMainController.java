...
public class RunMainController extends MainLayoutBasicController implements GenericEventListener, Activateable {
	...
	private final ICourse course;// o_clusterOK: this controller listen to course change events
	 ...
	 *            number: node with nodenumber will be activated "assessmentTool": assessment tool will be activated
	...
	private String getExtendedCourseTitle(final Locale locale) {
		...
		final RepositoryEntry repositoryEntry = RepositoryManager.getInstance().lookupRepositoryEntry(course, false);
	...
	private void doHandleToolEvents(final UserRequest ureq, final String cmd) {
		if (cmd.indexOf(CMD_START_GROUP_PREFIX) == 0) {
			...
			final BusinessGroup group = groupManager.loadBusinessGroup(groupKey, false);
			...
			// was brasato getWindowControl().getDTabs().activateStatic(ureq, RepositorySite.class.getName(),RepositoryMainController.JUMPFROMEXTERN +
			// RepositoryMainController.JUMPFROMCOURSE + courseRepositoryEntry.getKey().toString());
			...
			final ControllerCreator ctrlCreator = new ControllerCreator() {
				public Controller createController(final UserRequest lureq, final WindowControl lwControl) {
					...
					final CourseCalendarController calendarController = new CourseCalendarController(lureq, llwControl, course);
	...
	private ToolController initToolController(final Identity identity, final UserRequest ureq) {
		...
		if (waitingListGroups.size() > 0) {
			...
			while (iter.hasNext()) {
				final BusinessGroup group = (BusinessGroup) iter.next();
				final BusinessGroupManager businessGroupManager = BusinessGroupManagerImpl.getInstance();
				final int pos = businessGroupManager.getPositionInWaitingListFor(identity, group);
		...
		if (offerBookmark && !isGuest) {
			...
			final BookmarkManager bm = BookmarkManager.getInstance();

...
public class StatisticDisplayController extends BasicController {
	...
	private final Translator headerTranslator_;
	...
	public StatisticDisplayController(final UserRequest ureq, final WindowControl windowControl, final ICourse course, final IStatisticManager statisticManager) {
		...
		if (statisticManager == null) { throw new IllegalArgumentException("statisticManager must not be null"); }
		this.course = course;
		...
		setTranslator(Util.createPackageTranslator(statisticManager.getClass(), ureq.getLocale(),
				Util.createPackageTranslator(StatisticDisplayController.class, ureq.getLocale())));
	...
	protected Component createInitialComponent(final UserRequest ureq) {
		...
		final Package pkg = getStatisticManager().getClass().getPackage();
		final String fullPkgName = pkg.getName();
	...
	private TableController createTableController(final UserRequest ureq, final StatisticResult result) {
		...
		final IndentedStatisticNodeRenderer indentedNodeRenderer = new IndentedStatisticNodeRenderer(Util.createPackageTranslator(statisticManager.getClass(),
				ureq.getLocale()));
	...
	protected long getCourseRepositoryEntryKey() {
		...
		final RepositoryEntry re = RepositoryManager.getInstance().lookupRepositoryEntry(ores, false);
		long resid = 0;
		if (re != null) {
			resid = re.getKey();
		}
	...
	private Graph calculateNodeGraph(final UserRequest ureq, final int rowId) {
		...
		final String chartIntroStr = headerTranslator_.translate("statistic.chart.intro", new String[] { selectionInfo, getStatsSinceStr(ureq) });

...
public class IsCourseAdministratorFunction extends AbstractFunction {
	...
	public Object call(final Object[] inStack) {
		...
		final boolean isCourseAdmin = cgm.isIdentityCourseAdministrator(ident);
		if (Tracing.isDebugEnabled(IsCourseAdministratorFunction.class)) {
			Tracing.logDebug("identity " + ident.getName() + ", courseadministrator:" + isCourseAdmin + ", in course "
					+ getUserCourseEnv().getCourseEnvironment().getCourseResourceableId(), IsCourseAdministratorFunction.class);
		}

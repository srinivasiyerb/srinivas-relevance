...
public class OlatTopNavController extends BasicController /* TODO:PB:OLAT-4047 implements GenericEventListener */implements GenericEventListener {
	...
	private final SearchController searchC;
	...
	public void event(final UserRequest ureq, final Component source, final Event event) {
		 ...
		 * //TODO:PB:OLAT-4047 else if (source == permLink){ WindowControl current =
		 * (WindowControl)getWindowControl().getWindowBackOffice().getWindow().getAttribute("BUSPATH"); String buspath = current != null ?
		 * JumpInManager.getRestJumpInUri(current.getBusinessControl()) : "NONE"; String postUrl = Settings.getServerContextPathURI()+"/url/"+buspath; String
		 * deliciousPost = "http://del.icio.us/post?url="+postUrl; ControllerCreator ctrl = BaseFullWebappPopupLayoutFactory.createRedirectingPopup(ureq, deliciousPost);
		 * openInNewBrowserWindow(ureq, ctrl); return; }
		 */
		if (source == topNavVC) {

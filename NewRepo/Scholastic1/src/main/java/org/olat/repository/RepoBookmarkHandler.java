...
public class RepoBookmarkHandler implements BookmarkHandler {
	...
	/**
	 * @see org.olat.bookmark.handler.BookmarkLaunchHandler#tryToLaunch(org.olat.bookmark.Bookmark, org.olat.core.gui.UserRequest,
	 *      org.olat.core.gui.control.WindowControl)
	 */
	@Override
	public boolean tryToLaunch(final Bookmark bookmark, final UserRequest ureq, final WindowControl wControl) {
		final OLATResourceable reores = BookmarkManager.getInstance().getLaunchOlatResourceable(bookmark);
		final RepositoryManager rm = RepositoryManager.getInstance();

		final RepositoryEntry re = rm.lookupRepositoryEntry(reores.getResourceableId());
		if (re == null) {
		...
		if (!rm.isAllowedToLaunch(ureq, re)) {
			final Translator trans = Util.createPackageTranslator(Bookmark.class, ureq.getLocale());
	...
	public String createJumpInURL(final Bookmark bookmark) {
		...
		if (re != null) { return RepoJumpInHandlerFactory.buildRepositoryDispatchURI(re); }
		return null;
	}

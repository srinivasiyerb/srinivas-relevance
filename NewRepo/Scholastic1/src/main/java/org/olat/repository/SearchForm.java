...
public class SearchForm extends FormBasicController {
	 ...
	 */
	public Long getId() {
		if (!hasId()) { throw new AssertException("Should not call getId() if there is no id. Check with hasId() before."); }
	...
	public Set<String> getRestrictedTypes() {
		...
		if (limitTypes != null && limitTypes.length > 0) { return new HashSet<String>(Arrays.asList(limitTypes)); }

...
public class RepositoryDetailsController extends BasicController implements GenericEventListener {
	 ...
	 * @param ureq
	 */
	private void checkSecurity(final UserRequest ureq) {
		if (ureq.getUserSession().getRoles().isOLATAdmin()) {
			...
			final boolean isInstitutionalResourceManager = RepositoryManager.getInstance().isInstitutionalRessourceManagerFor(repositoryEntry, ureq.getIdentity());
	...
	private void updateRepositoryEntryView(final UserRequest ureq) {
		...
		final boolean isLaunchable = checkIsRepositoryEntryLaunchable(ureq);
		...
		final ImageComponent ic = RepositoryEntryImageController.getImageComponentForRepositoryEntry("image", repositoryEntry);
		...
		if (typeName != null) { // add image and typename code
			...
			if (repositoryEntry != null && RepositoryManager.getInstance().createRepositoryEntryStatus(repositoryEntry.getStatusCode()).isClosed()) {
				final PackageTranslator pT = new PackageTranslator(RepositoryEntryStatus.class.getPackage().getName(), ureq.getLocale());
		...
		// Where is it in use
		final String referenceDetails = ReferenceManager.getInstance().getReferencesToSummary(repositoryEntry.getOlatResource(), ureq.getLocale());
	...
	private void updateDetailsToolC(final UserRequest ureq) {
		...
		if (!isGuestOnly) {
			...
			if (repositoryEntry.getOlatResource().getResourceableTypeName().equals(CourseModule.getCourseTypeName()) && !(isOwner || isAuthor)) {
				canDownload = false;
			}
	...
	private boolean checkIsRepositoryEntryLaunchable(final UserRequest ureq) {
		...
		if (rm.isAllowedToLaunch(ureq, repositoryEntry) || (type.supportsLaunch(repositoryEntry) && ureq.getUserSession().getRoles().isOLATAdmin())) { return true; }// else
																																										// if(!rm.isAllowedToLaunch(ureq,
																																										// repositoryEntry)){
		return false;
		// }
		// OLAT-5571: noticed while reviewing OLAT-5571: below code was never called due to if statement above
		/*
		 * if (type instanceof CourseHandler) { // course ICourse course = CourseFactory.loadCourse(repositoryEntry.getOlatResource()); CourseNode rootNode =
		 * course.getRunStructure().getRootNode(); UserCourseEnvironmentImpl uce = new UserCourseEnvironmentImpl(ureq.getUserSession().getIdentityEnvironment(), course
		 * .getCourseEnvironment()); NodeEvaluation nodeEval = rootNode.eval(uce.getConditionInterpreter(), new TreeEvaluation()); boolean mayAccessWholeTreeUp =
		 * NavigationHandler.mayAccessWholeTreeUp(nodeEval); if (!mayAccessWholeTreeUp || !nodeEval.isVisible()) { String explan = rootNode.getNoAccessExplanation();
		 * String sExplan = (explan == null ? translate("launch.noaccess") : Formatter.formatLatexFormulas(explan)); main.contextPut("disabledlaunchreason", sExplan);
		 * return false; } } return repositoryEntry.getCanLaunch();
		 */
	}
	...
	void doEdit(final UserRequest ureq) {
		...
		if (!typeToEdit.supportsEdit(repositoryEntry)) { throw new AssertException("Trying to edit repository entry which has no assoiciated editor: " + typeToEdit); }
	...
	public void event(final UserRequest ureq, final Controller source, final Event event) {
		...
		if (source == groupEditController) {
			...
			if (event == Event.CHANGED_EVENT) {
				...
				this.repositoryEntry = (RepositoryEntry) DBFactory.getInstance().loadObject(repositoryEditDescriptionController.getRepositoryEntry()); // need a reload
																																						// from hibernate
																																						// because create
																																						// a new cp load a
																																						// repository-entry

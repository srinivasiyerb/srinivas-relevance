...
public class CPCourseNodeIndexer extends FolderIndexer implements CourseNodeIndexer {
	...
	public void doIndex(final SearchResourceContext repositoryResourceContext, final ICourse course, final CourseNode courseNode, final OlatFullIndexer indexWriter)
		...
		if (Tracing.isDebugEnabled(CPCourseNodeIndexer.class)) {
			Tracing.logDebug("Index Content Package... courseNodeCounter=" + courseNodeCounter++, CPCourseNodeIndexer.class);
		}
		...
		final RepositoryEntry re = CPEditController.getCPReference(courseNode.getModuleConfiguration(), true);

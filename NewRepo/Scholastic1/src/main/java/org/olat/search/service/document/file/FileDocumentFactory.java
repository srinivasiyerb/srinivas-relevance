...
public class FileDocumentFactory {
	...
	public static Document createDocument(final SearchResourceContext leafResourceContext, final VFSLeaf leaf) throws DocumentNotImplementedException, IOException,
		...
		if (PDF_SUFFIX.indexOf(suffix) >= 0) { return PdfDocument.createDocument(leafResourceContext, leaf); }
		if (HTML_SUFFIX.indexOf(suffix) >= 0) { return HtmlDocument.createDocument(leafResourceContext, leaf); }
		if (XML_SUFFIX.indexOf(suffix) >= 0) {
			if (IMS_MANIFEST_FILE.equals(fileName)) { return IMSMetadataDocument.createDocument(leafResourceContext, leaf); }
			return XmlDocument.createDocument(leafResourceContext, leaf);
		}
		if (TEXT_SUFFIX.indexOf(suffix) >= 0) { return TextDocument.createDocument(leafResourceContext, leaf); }

		// microsoft openxml
		if (suffix.indexOf(WORD_X_SUFFIX) >= 0) { return WordOOXMLDocument.createDocument(leafResourceContext, leaf); }
		if (suffix.indexOf(EXCEL_X_SUFFIX) >= 0) {
			if (excelFileEnabled) { return ExcelOOXMLDocument.createDocument(leafResourceContext, leaf); }
			return null;
		}
		if (suffix.indexOf(POWERPOINT_X_SUFFIX) >= 0) {
			if (pptFileEnabled) { return PowerPointOOXMLDocument.createDocument(leafResourceContext, leaf); }
			return null;
		}

		// microsoft
		if (WORD_SUFFIX.indexOf(suffix) >= 0) { return WordDocument.createDocument(leafResourceContext, leaf); }
		if (POWERPOINT_SUFFIX.indexOf(suffix) >= 0) {
			if (pptFileEnabled) { return PowerPointDocument.createDocument(leafResourceContext, leaf); }
			return null;
		}
		if (EXCEL_SUFFIX.indexOf(suffix) >= 0) {
			if (excelFileEnabled) { return ExcelDocument.createDocument(leafResourceContext, leaf); }
			return null;
		}

		// open document
		if (OD_TEXT_SUFFIX.indexOf(suffix) >= 0 || OD_SPREADSHEET_SUFFIX.indexOf(suffix) >= 0 || OD_PRESENTATION_SUFFIX.indexOf(suffix) >= 0
				|| OD_FORMULA_SUFFIX.indexOf(suffix) >= 0 || OD_GRAPHIC_SUFFIX.indexOf(suffix) >= 0) { return OpenDocument.createDocument(leafResourceContext, leaf); }

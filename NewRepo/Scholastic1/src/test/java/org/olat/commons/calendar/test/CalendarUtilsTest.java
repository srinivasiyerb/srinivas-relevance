...
public class CalendarUtilsTest extends OlatTestCase {
	...
	public void testListEventsForPeriod() {
		...
		System.out.println("*** Generate events in between " + new Date(kalendarStart) + " and "
		...
		final CalendarManager manager = CalendarManagerFactory.getJUnitInstance().getCalendarManager();
		...
		final List events = CalendarUtils.listEventsForPeriod(kalendar, new Date(kalendarStart),
				new Date(kalendarStart + (1000 * ((long) (goBackNumYears * oneYearSec)))));
	 ...
	 * @param cal
	 */
	private void createTestEvents(final int numberOfEvents, final Kalendar cal) {
		final Random rand = new Random();
		final long startUsed = RUNTIME.totalMemory() - RUNTIME.freeMemory();
		for (int i = 0; i < numberOfEvents; i++) {
			final long begin = kalendarStart + (1000 * ((long) rand.nextInt(goBackNumYears * oneYearSec)));
			final KalendarEvent event = new KalendarEvent("id" + i, "test" + i, new Date(begin), rand.nextInt(maxEventDuratio));
			cal.addEvent(event);
		}

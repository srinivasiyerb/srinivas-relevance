...
public class EmailCheckPerformanceTest extends OlatTestCase {
	...
	public void testUserManger() throws Exception {
		...
		for (int i = 1; i < MAX_LOOP; i++) {
			final Identity test = UserManager.getInstance().findIdentityByEmail(i + USERNAME_CONSTANT + TEST_DOMAIN);
			if (test == null) {
		...
		for (int i = 1; i < MAX_LOOP; i++) {
			final boolean test = UserManager.getInstance().userExist(i + USERNAME_CONSTANT + TEST_DOMAIN);
			assertTrue(test);
		}
	...
	private void createUsers() {
		...
		if (um.findIdentityByEmail("1" + USERNAME_CONSTANT + TEST_DOMAIN) == null) {
			...
			for (int i = 1; i < numberUsers + 1; i++) {
				...
				final User user = um.createUser(username + "first", username + "last", username + TEST_DOMAIN);

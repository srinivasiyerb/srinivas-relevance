	...
	public void event(final UserRequest ureq, final Controller source, final Event event) {
		...
		if (source == sepValInForm) {
			if (event == Event.DONE_EVENT) {
				...
				final PackageTranslator userTrans = new PackageTranslator("org.olat.user", ureq.getLocale());
				final PackageTranslator properyHandlersTrans = new PackageTranslator("org.olat.user.propertyhandlers", ureq.getLocale());
		...
		// preparing step 3
			...
			if (event == Event.DONE_EVENT) { // user clicked 'next'-button !!!!!!!
				...
				final BulkActionModule bam = new BulkActionModule();
				...
				for (int i = 0; i < sepValFinModel.getRowCount(); i++) {
					...
					if (selectedOlatKey == 0) { // matrikelnumber
						...
						final List identitiesFoundByInst = BaseSecurityManager.getInstance().getIdentitiesByPowerSearch(null, searchValue, true, null, null, null, null,
								null, null, null, null);
				...
				final List bulkResults = bam.apply(identities, ba);

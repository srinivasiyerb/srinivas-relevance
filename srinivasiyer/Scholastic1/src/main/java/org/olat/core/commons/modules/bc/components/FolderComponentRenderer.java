...
public class FolderComponentRenderer implements ComponentRenderer {
	...
	public void render(Renderer renderer, StringOutput target, Component source, URLBuilder ubu, Translator translator, RenderResult renderResult, String[] args) {
		...
		if (renderType == 1) target.append(crumbRenderer.render(fc, ubu, true, iframePostEnabled));
		else if (renderType == 2) target.append(crumbRenderer.render(fc, ubu, false, iframePostEnabled));
		else renderList(renderer, target, fc, ubu, translator, iframePostEnabled);
	}

...
class ContextHelpVisitor extends LogDelegator implements FileVisitor {
	 ...
	 * 
	 * @param basePathConfig The base path to be subtracted from the file name to get the classname
	 * @param contextHelpPagesLegacyLookupIndex hash map that offers a way to lookup the package of a page when only the page name is known (used for legacy help pages)
	 * @param allContextHelpPages set that contains all help pages pathes
	 * @param blacklist Set of page pathes that should not be indexed
	 */
		...
		if (file.isFile()) { // regular file
			...
			if (parentFile.getName().equals(CHELP_DIR)) {
				...
				try {
					...
					logError("Error while searching for context help file in a jar", e);
				}
			}
		}
		// else ignore
	...
	private void addHelpPageToList(String toBeChechedkFilName, String classPath) {
		...
		for (String blackListItem : blacklist) {
			if (combinedPath.startsWith(blackListItem)) return;
		}
		// already in the lookup index?
		if (allContextHelpPages.contains(combinedPath)) return;
		...
		if (contextHelpPagesLegacyLookupIndex.containsKey(toBeChechedkFilName)) {
			logDebug("Detected two help pages with the same name::" + toBeChechedkFilName + ", in package::" + contextHelpPagesLegacyLookupIndex.get(toBeChechedkFilName)

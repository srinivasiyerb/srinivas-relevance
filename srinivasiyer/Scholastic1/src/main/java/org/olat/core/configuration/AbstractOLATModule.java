...
public abstract class AbstractOLATModule extends LogDelegator implements GenericEventListener {
	...
	public final void init(Properties moduleConfig) {
		...
		if (moduleConfigProperties == null) throw new StartupException(
				"PersistedProperties is null. You have to set Persisted properties in spring config when you use abstractOlatModule!");
	...
	protected boolean getBooleanConfigParameter(String parameterName, boolean defaultValue) {
		String stringValue = moduleDefaultConfig.getProperty(parameterName);
		if ((stringValue != null) && (stringValue.trim().equalsIgnoreCase("TRUE") || stringValue.trim().equalsIgnoreCase("ENABLED"))) { return true; }
		if ((stringValue != null) && (stringValue.trim().equalsIgnoreCase("FALSE") || stringValue.trim().equalsIgnoreCase("DISABLED"))) { return false; }
		logInfo("Take default Boolean conf-parameter '" + parameterName + "', value=" + stringValue + ", only true/false supported => take default value.", null);
		return defaultValue;
	}

...
public class Emailer {
	...
	public Emailer(Identity mailFromIdentity, boolean tryInstitutionalEmail) {
		...
		footer = trans.translate("footer.with.userdata", new String[] { user.getProperty(UserConstants.FIRSTNAME, null), user.getProperty(UserConstants.LASTNAME, null),
				mailFromIdentity.getName(), institution, Settings.getServerContextPathURI() });
	...
	public boolean sendEmail(List<ContactList> listOfContactLists, String subject, String body, List<File> attachments) throws AddressException, MessagingException {
		/*
		 * if the mailhost was not set in the olat.local.properties, we assume that no emailing is wished.
		 */
		if (MailHelper.getMailhost() == null || MailHelper.getMailhost().equals("")
				|| ((MailHelper.getMailhost() instanceof String) && ((String) MailHelper.getMailhost()).equalsIgnoreCase("disabled"))) return false;
		...
		MailHelper.sendMessage(msg.getFrom()[0], msg.getRecipients(RecipientType.TO), msg.getRecipients(RecipientType.CC), msg.getRecipients(RecipientType.BCC), body
				+ footer, subject, attachmentsArray, result);

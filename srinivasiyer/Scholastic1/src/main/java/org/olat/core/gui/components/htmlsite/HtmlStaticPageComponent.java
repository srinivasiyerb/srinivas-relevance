...
public class HtmlStaticPageComponent extends Component implements AsyncMediaResponsible {
	...
	public MediaResource getAsyncMediaResource(UserRequest ureq) {
		...
		if (moduleURI != null) {
			...
			if (sourceItem == null || (sourceItem instanceof VFSContainer)) { return new NotFoundMediaResource(moduleURI); }
	...
	public void setCurrentURI(String currentURI) {
		if (!isFileTypeSupported(currentURI)) { throw new AssertException("can only accept files which are inline renderable(.html, .htm, .txt), but given filename is:"
				+ currentURI); }
		this.currentURI = currentURI;
		setDirty(true);

		VFSItem sourceItem = null;
		if (rootContainer != null) sourceItem = rootContainer.resolve(currentURI);

 ...
 * Creates an asynchronously delivered {@link org.olat.core.gui.media.ExcelMediaResource excel media resource}.</li>
...
public class TableController extends BasicController {
	 ...
	 * 
	 * @param tableConfig The table GUI configuration determines the tables behavior, may be <code>null</code> to use default table configuration.
	 ...
	 * @param noFilterOption The translated key for the no-filter filter or <code>null</code> if not used
	 ...
	 * 
	 * @param tableConfig The table gui configuration determines the tables behaviour, may be <code>null</code> to use default table config.
	 * @param ureq The user request
	 * @param wControl The window control
	 * @param tableTrans The translator that is used to translate the table
	 */
	...
	private void handleCommandsOfTableVcContainer(final UserRequest ureq, final Event event) {
		...
		if (cmd.equals("cmd.changecols") && tableConfig.getPreferencesKey() != null) {
			...
			// security check
			if (filters.size() < (filterPosition + 1)) { throw new AssertException("Filter size was ::" + filters.size() + " but requested filter was ::"
					+ filterPosition); }
			// update new filter value
	...
	/**
	 * @param activeFilter The currently applied filter or <code>null</code> if no filter is applied
	 */

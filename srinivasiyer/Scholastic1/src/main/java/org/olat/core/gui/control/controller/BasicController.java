...
public abstract class BasicController extends DefaultController {
	...
	protected BasicController(UserRequest ureq, WindowControl wControl, Translator fallBackTranslator) {
		...
		this.locale = ureq.getLocale();
		this.identity = ureq.getIdentity();
		if (fallBackTranslator == null) { throw new AssertException("please provide a fall translator if using this constructor!!"); }
		this.fallbackTranslator = fallBackTranslator;
		this.translator = Util.createPackageTranslator(this.getClass(), locale, fallBackTranslator);
		this.velocity_root = Util.getPackageVelocityRoot(this.getClass());
		this.mreg = MapperRegistry.getInstanceFor(ureq.getUserSession());
		this.logger = Tracing.createLoggerFor(this.getClass());
	}
	 ...
	 * @throws AssertException if the controller to be added is already contained.
	 */
	protected Controller listenTo(Controller controller) {
		controller.addControllerListener(this);
		if (childControllers == null) childControllers = new ArrayList<Controller>(4);
		/*
	 ...
	 * listenTo() method.
	 * 
	 * @param controller the controller to be disposed. When controller is NULL the method returns doing nothing, no exception will be thrown.
	 * @throws AssertException if the controller is not contained.
	 */
	 ...
	 * 
	 * @param cacheableMapperID the mapper ID that is used in the url to identify this mapper. Should be something that is derived from the context or resource that is
	 *            delivered by the mapper
	 * @param m the mapper that delivers the resources
	 * @return The mapper base URL
	 */
	...
	protected void deregisterMapper(Mapper m) {
		...
		if (!success) throw new AssertException("removing a mapper which was not registered");
		mreg.deregister(m);
	}
	 ...
	 * 
	 * @param page the velocity page to use in the _content folder, e.g. "index", or "edit". The suffix ".html" gets automatically added to the page name e.g.
	 *            "index.html".
	 */
	 ...
	 * 
	 * @param ureq
	 * @param windowContentCreator a creator which is used to create the windows content
	 */
	 ...
	 * </pre>
	 * 
	 * @param key the key to use (in the LocalStrings_curlanguage file of your controller)
	 */
	 ...
	 * @param arg the argument to pass in for the translation. if you have more than one argument, please use getTranslator().translate(....)
	 ...
	 * @param setLocaleOnTranslator true: the new locale is applied to the translator; false: the new locale is not applied to the translator
	 */
	...
	/**
	 * Log an error message to the system log file. An error is when something unexpected happened that could not be resolved and as a result the current workflow has to
	 * be terminated.
	 * <p>
	 * Alternatively it is possible to throw an OLATRuntimeException if your code does not present a special alternative GUI path for the user.
	 * <p>
	 * If you can, log the error and present a useful screen to the user.
	 * 
	 * @param logMsg Human readable log message
	 * @param cause stack trace or NULL
	 */
	...
	/**
	 * Log a warn message to the system log file. A warn message should be used when something unexpected happened but the system somehow can deal with it. e.g by using
	 * default values.
	 * 
	 * @param logMsg Human readable log message
	 * @param cause stack trace or NULL
	...
	/**
	 * Log a debug message to the system log file. A debug message is something that can be useful to get more relevant information in a deployed system. Thus, think very
	 * carefully about which information could be useful to debug a live system. Be very verbose.
	 * <p>
	 * Debug messages are not meant for debugging while development phase, use your debugger for this purpose. The only purpose of logDebug is to debug a live system.
	 * <p>
	 * To prevent many expensive string concatenation in a live system where the log level for this class is not set to debug you must use logDebug always in conjunction
	 * with isLogDebug(): <code>
	 ...
	 * 
	 * @param logMsg Human readable log message
	 * @param userObj A user object with additional information or NULL
	 */
	...
	/**
	 * Log an info message to the system log file. Info messages are useful to log configuration settings or very important events to the logfile. Log only information
	 * that is really important to have in the logfile.
	 * <p>
	 * If the information is only useful to debug a problem, you might consider using logDebug instead
	 * 
	 * @param logMsg Human readable log message
	 * @param userObj A user object with additional information or NULL
	 */
	...
	/**
	 * Log an audit message to the system log file. An audit message contains information about a user behavior: a user logged into the system, a user did a critical
	 * action.
	 * 
	 * @param logMsg Human readable log message
	 * @param userObj A user object with additional information or NULL

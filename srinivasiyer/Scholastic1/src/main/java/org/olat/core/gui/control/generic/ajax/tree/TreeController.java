...
public class TreeController extends BasicController {
	 ...
	 * @param titleRootNode Title displayed on the root node
	 * @param dataModel The dynamic datamodel for this tree
	 * @param nodeDragOverCallback Javascript method used as callback when a node is draged and moved around over the tree. The method must be defined somewhere else in
	 *            your code, in this case you provide the method name. Alternatively you cen define the method right here in the string with function(event){}. Use NULL
	 *            if not used.
	 */
	 ...
	 * 
	 * @param doSort true: sorting enabled; false: no sorting
	 * @param asc true: sort ascending; false: sort descending
	 * @param foldersFirst true: show sorted folder items first (items with attribute 'isTypeLeaf') and then sorted leafs
	 */
	public void setTreeSorting(boolean doSort, boolean asc, boolean foldersFirst) {
		if ((asc || foldersFirst) && !doSort) { throw new AssertException("Programming error: can not sort ascending or folderFirst when sorting flag is set to false"); }
	 ...
	 * 
	 * @param treeEditingEnabled true: enable tree editing; false: disable tree editing
	 * @param beforeStartEditCallback Javascript method used as callback when a node editor is started. The method must be defined somewhere else in your code, in this
	 *            case you provide the method name. Alternatively you cen define the method right here in the string with function(event){}. Use NULL if not used. The
	 *            function can return false to cancel the edit request (e.g. to implement non-editable nodes)
	 * @param beforeCompleteCallback Javascript method used as callback when a node is edited and the user closes the editor to save the changes. The method must be
	 *            defined somewhere else in your code, in this case you provide the method name. Alternatively you can define the method right here in the string with
	 *            function(event){}. Use NULL if not used. The function can return false to stop the edit request and revert to the original state (e.g. to check for
	 *            valid new values)
	 */
	public void setTreeInlineEditing(boolean treeEditingEnabled, String beforeStartEditCallback, String beforeCompleteCallback) {
		...
		if (beforeCompleteCallback != null) mainVC.contextPut("beforeCompleteCallback", beforeCompleteCallback);
	}

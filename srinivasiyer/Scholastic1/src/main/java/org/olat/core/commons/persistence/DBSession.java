...
class DBSession {
	...
	protected void clearTransaction() {
		if ((hibernateSession != null) && hibernateSession.isOpen()) { throw new DBRuntimeException("clearTransaction expected session to be closed at this point!"); }
		trxWrapper = null;
	}
	...
	protected void close() {
		try {
			if ((hibernateSession != null) && hibernateSession.isOpen()) {
				...
				if ((getTransaction() != null) && getTransaction().isInTransaction() && !getTransaction().isRolledBack()) { throw new AssertException(
						"Try to close un-committed session"); }

...
public class IFrameDisplayController extends BasicController implements GenericEventListener {
	 ...
	 * @param wControl
	 * @param fileRoot
	 * @param ores - send an OLATresourcable of the context (e.g. course) where the iframe runs and it will be checked if the user has textmarking (glossar) enabled in
	 *            this course
	 */
	 ...
	 * @param forceLoading true: force rendering of iframe velocity container, will redraw everything; false: set new uri without redrawing iframe. This implies that the
	 *            content in the iframe has already been loaded (e.g. by an in line user click)
	 */
	...
	private boolean isPageDownloadAllowed(String uri) {
		if (!allowDownload || !StringHelper.containsNonWhitespace(uri)) { return false; }
		String uriLc = uri.toLowerCase();
		if (uriLc.endsWith(".html") || uriLc.endsWith(".htm") || uriLc.endsWith(".xhtml")) { return false; }
		return true;
	}
	...
	private String guessContentType(Page page, String content) {
		...
		if (cType == null) { return DEFAULT_CONTENT_TYPE; }
		return cType;
	}
	...
	private String guessEncoding(String content) {
		...
		if (m.find()) {
			...
			// if longer than 50 the regexp did fail
			if (htmlcharset.length() < 50) { return htmlcharset; }
		}
		...
		if (xmlDeclaration.find()) {
			...
			// if longer than 50 the regexp did fail
			if (xmlcharset.length() < 50) { return xmlcharset; }
		}
	 ...
	 * 
	 * @param page
	 * @param addCheckForInlineEvents true: check if page is rendered in iframe, if yes send event to framework; false: don't do this check
	 * @return
	 */
	/**
	...
	private String injectJavaScript(String page, String mimetype, boolean addCheckForInlineEvents) {
		...
		if (!parser.isValidHtml()) { return page; }

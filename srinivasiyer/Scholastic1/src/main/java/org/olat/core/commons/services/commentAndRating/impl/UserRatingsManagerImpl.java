...
public class UserRatingsManagerImpl extends UserRatingsManager {
	...
	/**
	 * Spring constructor. Use the getInstance method instead of calling this constructor directly.
	 */
	...
	public Float calculateRatingAverage() {
		...
		if (getOLATResourceableSubPath() == null) {
			...
			query = DBFactory.getInstance().createQuery("select avg(rating) from UserRatingImpl where resName=:resname AND resId=:resId AND resSubPath is NULL");
		} else {
			query = DBFactory.getInstance().createQuery("select avg(rating) from UserRatingImpl where resName=:resname AND resId=:resId AND resSubPath=:resSubPath");
	...
	public Long countRatings() {
		...
		if (getOLATResourceableSubPath() == null) {
			// special query when sub path is null
			query = DBFactory.getInstance().createQuery("select count(*) from UserRatingImpl where resName=:resname AND resId=:resId AND resSubPath is NULL");
		} else {
			query = DBFactory.getInstance().createQuery("select count(*) from UserRatingImpl where resName=:resname AND resId=:resId AND resSubPath=:resSubPath");
			query.setString("resSubPath", getOLATResourceableSubPath());
		}
		query.setString("resname", getOLATResourceable().getResourceableTypeName());
		query.setLong("resId", getOLATResourceable().getResourceableId());
	...
	public UserRating createRating(Identity creator, int ratingValue) {
		UserRating rating = new UserRatingImpl(getOLATResourceable(), getOLATResourceableSubPath(), creator, Integer.valueOf(ratingValue));
	...
	public int deleteRating(UserRating rating) {
		if (!isRatingOfResource(rating)) { throw new AssertException("This user rating manager is initialized for another resource than the given comment."); }
	...
	private boolean isRatingOfResource(UserRating originalRating) {
		if (this.getOLATResourceable().getResourceableId().equals(originalRating.getResId())
				&& this.getOLATResourceable().getResourceableTypeName().equals(originalRating.getResName())) {
			// check on resource subpath: can be null
			if (this.getOLATResourceableSubPath() == null) {
				return (originalRating.getResSubPath() == null);
			} else {
				return this.getOLATResourceableSubPath().equals(originalRating.getResSubPath());
			}
		}
		return false;
	}

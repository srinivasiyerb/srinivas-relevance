...
public class SelectionTreeRenderer implements ComponentRenderer {
		...
		for (int i = 0; i < childcnt; i++) {
			...
			if (child.isAccessible()) {
				if (multiselect) { // render chekcboxes
					...
					sb.append("\" ");
					if (child.isSelected()) sb.append("checked ");
					sb.append("value=\"");
	...
	private void renderNodeIcon(StringOutput sb, TreeNode node) {
		...
		if (iconCssClass != null) {
			...
			if (deco1 != null) sb.append("<span class=\"").append(deco1).append("\"></span>");

			String deco2 = node.getIconDecorator2CssClass();
			if (deco2 != null) sb.append("<span class=\"").append(deco2).append("\"></span>");

			String deco3 = node.getIconDecorator3CssClass();
			if (deco3 != null) sb.append("<span class=\"").append(deco3).append("\"></span>");

			String deco4 = node.getIconDecorator4CssClass();
			if (deco4 != null) sb.append("<span class=\"").append(deco4).append("\"></span>");

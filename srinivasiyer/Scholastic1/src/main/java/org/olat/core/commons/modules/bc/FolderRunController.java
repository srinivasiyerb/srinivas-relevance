...
public class FolderRunController extends BasicController implements Activateable {
	 ...
	 * 
	 * @param rootContainer The folder base. User can not navigate out of this container.
	 * @param displayWebDAVLink true: show the webDAV link; false: hide the webDAV link
	 * @param displaySearch true: display the search field; false: omit the search field. Note: for guest users the search is always omitted.
	 ...
	 */
	public FolderRunController(VFSContainer rootContainer, boolean displayWebDAVLink, boolean displaySearch, UserRequest ureq, WindowControl wControl,
			VFSItemFilter filter, CustomLinkTreeModel customLinkTreeModel) {
		...
		if (WebDAVManager.getInstance().isEnabled() && displayWebDAVLink) folderContainer.contextPut("webdavlink", FolderManager.getWebDAVLink());
			...
			if (event == FolderCommand.FOLDERCOMMAND_FINISHED) {
				...
				if (source instanceof CmdCreateFile) {
					ThreadLocalUserActivityLogger.log(
							...
							getClass(),
							CoreLoggingResourceable.wrapBCFile(folderComponent.getCurrentContainerPath()
									+ ((folderComponent.getCurrentContainerPath().length() > 1) ? File.separator : "") + ((CmdCreateFile) source).getFileName()));
				} else if (source instanceof CmdCreateFolder) {
					ThreadLocalUserActivityLogger.log(
							FolderLoggingAction.FOLDER_CREATE,
							getClass(),
							CoreLoggingResourceable.wrapBCFile(folderComponent.getCurrentContainerPath()
									+ ((folderComponent.getCurrentContainerPath().length() > 1) ? File.separator : "") + ((CmdCreateFolder) source).getFolderName()));
				} else if (source instanceof CmdEditContent) {
					ThreadLocalUserActivityLogger.log(
							FolderLoggingAction.FILE_EDIT,
							getClass(),
							CoreLoggingResourceable.wrapBCFile(folderComponent.getCurrentContainerPath()
									+ ((folderComponent.getCurrentContainerPath().length() > 1) ? File.separator : "") + ((CmdEditContent) source).getFileName()));
					...
					while (it.hasNext()) {
						...
						ThreadLocalUserActivityLogger.log(
								...
								getClass(),
								CoreLoggingResourceable.wrapBCFile(folderComponent.getCurrentContainerPath()
										+ ((folderComponent.getCurrentContainerPath().length() > 1) ? File.separator : "") + aFileName));
					}
	...
	/**
	 * @seec org.olat removeAsListenerAndDispose(folderCommandController); folderCommandController = null; removeAsListenerAndDispose(cmc); cmc = null; .UserRequest,
	...
	private String getFormAction(UserRequest ureq) {
		...
		while (params.hasMoreElements()) {
			...
			if (key.startsWith(ACTION_PRE)) { return key.substring(ACTION_PRE.length()); }
		}
		return null;
	}

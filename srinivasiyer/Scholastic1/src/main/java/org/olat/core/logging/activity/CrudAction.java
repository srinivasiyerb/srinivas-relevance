		...
		if (fields != null) {
			...
			for (int i = 0; i < fields.length; i++) {
				...
				if (field.getType() == CrudAction.class) {
					...
					if (initials.contains(initial)) { throw new IllegalStateException("CrudAction contained at least two actions with the same initial '" + initial
							+ "'. One of which is " + field.getName()); }

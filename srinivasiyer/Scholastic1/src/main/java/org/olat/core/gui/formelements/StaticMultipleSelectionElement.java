...
public class StaticMultipleSelectionElement extends AbstractFormElement implements MultipleSelectionElement {
	...
	public void setValues(String[] values) {
		...
		for (int i = 0; i < values.length; i++) {
			...
			if (!foundKey) throw new AssertException("submitted key '" + key + "' was not found in the keys of formelement named " + this.getName() + " , keys="
					+ Arrays.asList(keys));
			selected.add(key);
		}
	}
	...
	/**
	 * @see org.olat.core.gui.formelements.MultipleSelectionElement#isAtLeastSelected(int, java.lang.String)
	 */
	@Override

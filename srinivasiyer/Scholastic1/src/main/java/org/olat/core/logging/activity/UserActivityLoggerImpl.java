...
public class UserActivityLoggerImpl implements IUserActivityLogger {
	...
	private void initWithRequest(HttpServletRequest hReq) {
		if (hReq == null) { throw new IllegalArgumentException("hReq must not be null"); }
	...
	private UserActivityLoggerImpl(IUserActivityLogger userActivityLogger, WindowControl wControl, boolean propageToThreadLocal) {
		if (userActivityLogger == null) { throw new IllegalArgumentException("userActivityLogger must not be null"); }
	...
	public static UserActivityLoggerImpl setupLoggerForController(WindowControl wControl) {
		...
		if (!(threadLocalUserActivityLogger instanceof UserActivityLoggerImpl)) { throw new IllegalStateException(
				"threadLocalUserActivityLogger must be of type GenericUserActivityLogger"); }
	 ...
	 * <p>
	 * 
	 * @param controller the Controller from which the IUserActivityLogger and the WindowControl (businessPath) should be extracted
	...
	public void addLoggingResourceInfo(ILoggingResourceable loggingResourceable) {
		if (loggingResourceable == null) { throw new IllegalArgumentException("resourceInfo must not be null"); }
	...
	public void frameworkSetBusinessPath(String businessPath) {
		if (businessPath == businessPath_ || businessPath == null || businessPath.length() == 0
				|| (businessPath_ != null && businessPath.length() < businessPath_.length())) { return; }
	...
	public void frameworkSetBCContextEntries(List<ContextEntry> bcEntries) {
		if (bcContextEntries_ == bcEntries || bcEntries == null || bcEntries.size() == 0 || (bcContextEntries_ != null && bcEntries.size() < bcContextEntries_.size())) { return; }
		if (bcEntries != null) {
		...
		if (bcContextEntries_ != null) {
			...
			for (Iterator<ContextEntry> it = bcContextEntriesCopy.iterator(); it.hasNext();) {
				...
				if (!foundIt) {
					...
					log_.info("Could not find any LoggingResourceable corresponding to this ContextEntry: " + ce.toString() + ", ce.getOLATResourceable()="
							+ ce.getOLATResourceable() + ", ce.getOLATResourceable().getOlatResource()=" + oresourceableOres + ", dump of resource infos:");
					for (Iterator<ILoggingResourceable> it2 = inputCopy.iterator(); it2.hasNext();) {
						ILoggingResourceable resourceInfo = it2.next();
						log_.info("id: " + resourceInfo.getId() + ", name=" + resourceInfo.getName() + ", type=" + resourceInfo.getType() + ", toString: "
								+ resourceInfo.toString());
					}
					log_.error("Could not find any LoggingResourceable corresponding to this ContextEntry: " + ce.toString(), new Exception(
							"UserActivityLoggerImpl.getCombinedOrderedLoggingResourceables()"));
				}
			}
		}
	...
	public void log(ILoggingAction loggingAction, Class<?> callingClass, ILoggingResourceable... lriOrNull) {
		...
		if (isLogAnonymous_ && actionType.equals(ActionType.tracking)) { return; }
		...
		if (loggingAction.getTypeListDefinition() == null) {
			...
			if (errorMsg != null) {
				...
				log_.warn("LoggingAction reported an inconsistency: " + loggingAction.getActionVerb() + " " + loggingAction.getActionObject() + ", action="
		...
		if (session_ == null) {
			...
			log_.error("No session available to UserActivityLogger. Cannot write log entry: " + crudAction.name() + ":" + actionVerb.name() + ", " + actionObject + ", "
		...
		if (session_.getSessionInfo() == null || session_.getSessionInfo().getSession() == null || session_.getSessionInfo().getSession().getId() == null
			...
			log_.error("No session information available to UserActivityLogger. Cannot write log entry: " + crudAction.name() + ":" + actionVerb.name() + ", "
					+ actionObject + ", " + convertLoggingResourceableListToString(resourceInfos), new Exception());
			return;
		}
		...
		if (identity == null) {
			...
			log_.error("No identity available to UserActivityLogger. Cannot write log entry: " + crudAction.name() + ":" + actionVerb.name() + ", " + actionObject + ", "
					+ convertLoggingResourceableListToString(resourceInfos), new Exception());
			return;
		}
		...
		if (lastLogObj != null) {
			...
			if (db != null && db.isError()) {
				...
				log_.warn("log: DB is in Error state therefore the UserActivityLoggerImpl cannot update the simpleDuration of log_id " + lastLogObj.getKey()
						+ " with value " + duration + ", loggingObject: " + lastLogObj);
			} else {
		...
		for (Iterator<String> iterator = userProperties_.iterator(); iterator.hasNext();) {
			...
			if (shorten) {
				log_.error("Userproperty was too long for logging-table (shortened automatically). check that nothing valueable is lost! value before cut: "
						+ userPropString);

...
public class UserCommentsManagerImpl extends UserCommentsManager {
	...
	/**
	 * @see org.olat.core.commons.services.commentAndRating.UserCommentsManager#createCommentManager(org.olat.core.id.OLATResourceable, java.lang.String)
	 */
	@Override
	protected UserCommentsManager createCommentManager(OLATResourceable ores, String subpath) {
	...
	public Long countComments() {
		...
		if (getOLATResourceableSubPath() == null) {
			...
			query = DBFactory.getInstance().createQuery("select count(*) from UserCommentImpl where resName=:resname AND resId=:resId AND resSubPath is NULL");
		} else {
			query = DBFactory.getInstance().createQuery("select count(*) from UserCommentImpl where resName=:resname AND resId=:resId AND resSubPath=:resSubPath");
	...
	/**
	 * @see org.olat.core.commons.services.commentAndRating.UserCommentsManager#createComment(org.olat.core.id.Identity, java.lang.String)
	 */
	@Override
	public UserComment createComment(Identity creator, String commentText) {
		UserComment comment = new UserCommentImpl(getOLATResourceable(), getOLATResourceableSubPath(), creator, commentText);
	...
	public UserComment replyTo(UserComment originalComment, Identity creator, String replyCommentText) {
		if (!isCommentOfResource(originalComment)) { throw new AssertException("This user comment manager is initialized for another resource than the given comment."); }
		...
		UserCommentImpl reply = new UserCommentImpl(getOLATResourceable(), getOLATResourceableSubPath(), creator, replyCommentText);
	...
	public List<UserComment> getComments() {
		...
		if (getOLATResourceableSubPath() == null) {
			// special query when sub path is null
			query = DBFactory.getInstance().createQuery("select comment from UserCommentImpl as comment where resName=:resname AND resId=:resId AND resSubPath is NULL");
		} else {
			query = DBFactory.getInstance().createQuery(
	...
	public UserComment updateComment(UserComment comment, String newCommentText) {
		if (!isCommentOfResource(comment)) { throw new AssertException("This user comment manager is initialized for another resource than the given comment."); }
		// First reload parent from cache to prevent stale object or cache issues
	...
	private boolean isCommentOfResource(UserComment originalComment) {
		if (this.getOLATResourceable().getResourceableId().equals(originalComment.getResId())
				&& this.getOLATResourceable().getResourceableTypeName().equals(originalComment.getResName())) {
			// check on resource subpath: can be null
			if (this.getOLATResourceableSubPath() == null) {
				return (originalComment.getResSubPath() == null);
			} else {
				return this.getOLATResourceableSubPath().equals(originalComment.getResSubPath());
			}
		}
		return false;
	}

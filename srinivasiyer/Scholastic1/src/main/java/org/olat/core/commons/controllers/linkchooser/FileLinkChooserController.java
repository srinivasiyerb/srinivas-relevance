...
public class FileLinkChooserController extends BasicController {
	 ...
	 * @param fileName the path of the file currently edited (in order to compute the correct relative paths for links), e.g. bla/blu.html or index.html
	 */
	...
	private boolean isFileSuffixOk(String fileName) {
		if (suffixes == null) {
			...
			for (String allowedSuffix : suffixes) {
				if (allowedSuffix.equals(suffix)) { return true; }
			}
		}
		return false;
	}

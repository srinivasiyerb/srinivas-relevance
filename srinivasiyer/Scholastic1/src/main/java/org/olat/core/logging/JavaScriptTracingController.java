...
public class JavaScriptTracingController extends BasicController {
	...
	public JavaScriptTracingController(UserRequest ureq, WindowControl wControl) {
		...
		Translator trans = Util.createPackageTranslator(JavaScriptTracingController.class, ureq.getLocale());
		// dummy velocity container, empty
		VelocityContainer mainVC = new VelocityContainer("JSTracing", Util.getPackageVelocityRoot(JavaScriptTracingController.class) + "/JSTracing.html", trans, this);
		...
		// JS Tracing
		String jsHeader = "<script type='text/javascript'>o_info.JSTracingUri='" + mapperUri + "/';o_info.JSTracingLogDebugEnabled=" + isLogDebugEnabled() + ";</script>";

...
public abstract class OLATResourceableEvent extends MultiUserEvent {
	...
	public boolean targetEquals(OLATResourceable ores, boolean exceptIfFalse) {
		...
		if (!res && exceptIfFalse) throw new AssertException("expected ores to be the same as target, but failed: ores =" + ores.getResourceableTypeName() + " / "
				+ ores.getResourceableId() + ", but event was for " + derived);
		return res;
	}

...
public class Window extends Container {
					...
					try {
						...
						if (target != null) { // the target was found
							...
							if (!validForDispatching && Tracing.isDebugEnabled(this.getClass())) {
								Tracing.logDebug("Invalid timestamp: ureq.compid:" + ureq.getComponentID() + " ureq.win-ts:" + ureq.getTimestampID() + " ureq.comp-ts:"
										+ ureq.getComponentTimestamp() + " target.timestamp:" + cTimest + " target=" + target, this.getClass());
							}
						} else {
						...
						if (didDispatch || !validForDispatching) {
							...
							Tracing.logDebug(
									"Found a valid timestamp but could not dispatch to component: ureq.compid:" + ureq.getComponentID() + " ureq.win-ts:"
											+ ureq.getTimestampID() + " ureq.comp-ts:" + ureq.getComponentTimestamp() + " target.timestamp:" + target.getTimestamp()
											+ " target=" + target, this.getClass());
							String reRenderUri = buildURIFor(this, timestampID, null);
							Command rmrcom = CommandFactory.createParentRedirectTo(reRenderUri);
							wbackofficeImpl.sendCommandTo(rmrcom);
						}
	 ...
	 * @param value the value, must not be NULL. Use removeAttribute() to remove a key
	 */
				...
				try {
					...
					synchronized (render_mutex) { // o_clusterOK by:fj
						...
						for (int i = 0; i < dCnt; i++) {
							...
							if (!wasDomR) { throw new AssertException("cannot replace as dom fragment:" + toRender.getComponentName() + " ("
									+ toRender.getClass().getName() + ")," + toRender.getExtendedDebugInfo()); }

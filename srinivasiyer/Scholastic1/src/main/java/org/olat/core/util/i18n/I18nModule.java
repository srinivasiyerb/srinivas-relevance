...
public class I18nModule extends AbstractOLATModule implements Destroyable {
	...
	private void doInit() {
		...
		if (transToolEnabled) {
			...
			if (!transToolEnabled) {
				logWarn("Translation configuration enabled but invalid translation tool source path defined. Disabling translation tool. Fix your configuration in spring config of i18Module",
						null);
		...
		logInfo("Configured i18nModule with default language::" + getDefaultLocale().toString() + " and the reference languages '" + referenceLanguagesConfig
	...
	private void doInitAvailableLanguages() {
		...
		for (String languageCode : i18nMgr.searchForAvailableLanguages(LANG_PACKS_DIRECTORY)) {
			if (availableLanguages.contains(languageCode)) {
				logWarn("Skipping duplicate or previously loaded  language::" + languageCode + " found in " + LANG_PACKS_DIRECTORY.getAbsolutePath(), null);
				continue;
			}
		...
		for (String langKey : availableLanguages) {
			...
			if (locale == null) {
				logError("Could not create locale for lang::" + langKey + ", skipping language and remove it from list of available languages", null);
	...
	private void doInitLanguageConfiguration() {
		...
		if (newDefaultLocale == null) {
			logError("Could not set default locale to value::" + defaultLanguageKey + " - no such language found. Using fallback locale instead", null);
			...
			logError("Did not find the default language::" + newDefaultLocale.toString() + " in the available availableLanguages files! Using fallback locale instead",
					null);
			newDefaultLocale = allLocales.get(transToolReferenceLanguages.get(0));
		}
		...
		if (!enabledLanguagesKeys.contains(getDefaultLocale().toString())) {
			...
			logWarn("The configured default language::" + defLang + " is not in the list of enabled languages. Enabling language::" + defLang, null);
		}
	}
	...
	/**
	 * @return The bundle name that contains the commonly used translations from the olatcore framework
	 */
	...
	/**
	 * search for bundles that contain i18n files. Searches in the org.olat.core package
	 */

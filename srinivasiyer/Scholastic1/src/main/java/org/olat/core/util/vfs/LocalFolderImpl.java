...
public class LocalFolderImpl extends LocalImpl implements VFSContainer {
	...
	protected LocalFolderImpl(File folderfile, VFSContainer parent) {
		...
		if (!alreadyExists && !succesfullCreated) throw new AssertException("Cannot create directory of LocalFolderImpl with reason (exists= ): " + alreadyExists
				+ " && created= " + succesfullCreated + ") path: " + folderfile.getAbsolutePath());
	}
	...
	public List<VFSItem> getItems(VFSItemFilter filter) {
		...
		if (!aFolder.isDirectory()) { throw new AssertException("basefile is not a directory: " + aFolder.getAbsolutePath()); }
	...
	private VFSStatus copyFrom(VFSItem source, boolean checkQuota) {
		...
		if (resolve(sourcename) != null) return VFSConstants.ERROR_NAME_ALREDY_USED;
		...
		if (source instanceof VFSContainer) {
			...
			if (VFSManager.isSelfOrParent(sourcecontainer, this)) return VFSConstants.ERROR_OVERLAPPING;
			...
			if (checkQuota) {
				...
				if (quotaLeft != Quota.UNLIMITED && quotaLeft < (s.getSize() / 1024)) return VFSConstants.ERROR_QUOTA_EXCEEDED;
			}

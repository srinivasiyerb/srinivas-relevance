...
public interface WindowManager extends Disposable {
	...
	/**
	 * locates and returns the resource found under <package-of-baseclass>/relpath; e.g. for baseClass org.olat.demo.MyDemo and relPath /js/myfunc.js ->
	 * org/olat/demo/_static/js/myfunc.js
	 * 
	 ...
	 * please see the {@link org.olat.core.gui.components.link.Link} on how to define a link to act as a popup window link. Once such a link is clicked the listening
	 * controller uses the <code>popupBrowserWindow = getWindowControl().getWindowBackOffice().getWindowManager().createNewPopupBrowserWindowFor(ureq, this);</code> to
	 * create the popup window (<code>this</code> may be any ControllerCreator!). Just call the method {@link PopupBrowserWindow#open(UserRequest)} to
	 * activate/open/render the content in the new browserwindow. <br>
	 * 
	 * @param ureq

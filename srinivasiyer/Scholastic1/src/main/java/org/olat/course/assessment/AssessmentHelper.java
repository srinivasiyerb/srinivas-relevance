...
public class AssessmentHelper {
	 ...
	 * @param courseNode an assessable course node or null if no details and attempts must be fetched
	...
	public static AssessedIdentityWrapper wrapIdentity(final Identity identity, final Map<Long, UserCourseEnvironment> localUserCourseEnvironmentCache,
		...
		if (uce == null) {
			...
			if (Tracing.isDebugEnabled(AssessmentHelper.class)) {
				Tracing.logDebug("localUserCourseEnvironmentCache hit failed, adding course environment for user::" + identity.getName(), AssessmentHelper.class);
			}
		}
	 ...
	 * Wraps an identity and it's score evaluation / attempts in a wrapper object for a given course node
	 * 
	 * @param uce The users course environment. Must be initialized (uce.getScoreAccounting().evaluateAll() must be called previously)
	 * @param courseNode an assessable course node or null if no details and attempts must be fetched
	 * @return a wrapped identity
	 */
	 ...
	 */
	static UserCourseEnvironment createAndInitUserCourseEnvironment(final Identity identity, final ICourse course) {
	...
	public static boolean checkForAssessableNodes(final CourseNode node) {
		if (checkIfNodeIsAssessable(node)) { return true; }
		// check children now
	 ...
	 * 
	 * @param editorModel
	 * @param excludeNode Node that should be excluded in the list, e.g. the current node or null if all assessable nodes should be used
	 ...
	 */
	static List<Map<String, Object>> addAssessableNodeAndDataToList(final int recursionLevel, final CourseNode courseNode, final UserCourseEnvironment userCourseEnv,
			final boolean discardEmptyNodes, final boolean discardComments) {

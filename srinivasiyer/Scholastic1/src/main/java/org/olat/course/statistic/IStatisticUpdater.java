...
public interface IStatisticUpdater {
	 ...
	 * 
	 * @param fullRecalculation when set to true the statisticupdater should do a complete recalc of all stats, rather than updating since the last call to
	 *            updateStatistic
	 * @param from update the statistics starting at the given from date - note that this is never null irrespective of fullRecalculation true or false
	 * @param until update the statistics ending at the given until date - this is never null
	 * @param statisticUpdateManager the StatisticUpdateManager is passed to the IStatisticUpdater for callbacks and utility functions such as access to the lastUpdated
	 *            property
	 */

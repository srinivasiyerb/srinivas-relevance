...
public class StatisticResult implements TableDataModel {
	...
	private void doAddQueryListResultsForNodeAndChildren(final CourseNode node, final List<?> result, final Set<String> groupByKeys) {
		...
		for (final Iterator<?> it = result.iterator(); it.hasNext();) {
			...
			if (columns.length != 3) { throw new IllegalStateException("result should be three columns wide"); }
	...
	private Map<String, Object> getIndentednodeRendererMap(final int row) {
		if (row >= orderedNodesList_.size()) { throw new IllegalStateException("row count too big: " + row + ", only having " + orderedNodesList_.size() + " elements"); }
	...
	public Object getValueAt(final int row, final int col) {
		if (row - 1 >= orderedNodesList_.size()) { return null; }
		if (row == orderedNodesList_.size()) {
			// that's the "total" row
			if (col == 0) { return TOTAL_ROW_TITLE_CELL; }
		...
		if (col == 0) { return getIndentednodeRendererMap(row); }
		...
		if (statisticMap == null) { return null; }

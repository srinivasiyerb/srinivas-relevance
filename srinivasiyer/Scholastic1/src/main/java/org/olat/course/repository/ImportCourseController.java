...
public class ImportCourseController extends BasicController implements IAddController {
	...
	public void repositoryEntryCreated(final RepositoryEntry re) {
		...
		final CourseEditorTreeNode editorRootNode = ((CourseEditorTreeNode) course.getEditorTreeModel().getRootNode());
	...
	private void processSharedFolder(final UserRequest ureq) {
		...
		if (sharedFolderImportController == null) {
			final RepositoryEntryImportExport sfImportExport = SharedFolderManager.getInstance().getRepositoryImportExport(getExportDataDir(course));
	...
	private void processGlossary(final UserRequest ureq) {
		...
		if (glossaryImportController == null) {
			final RepositoryEntryImportExport sfImportExport = GlossaryManager.getInstance().getRepositoryImportExport(getExportDataDir(course));
	...
	private void cleanupExportDataDir() {
		...
		final File fExportedDataDir = getExportDataDir(course);
	 ...
	 * @param nl
	 */
	public static void collectNodesAsList(final CourseEditorTreeNode rootNode, final List nl) {
		nl.add(rootNode);
	...
	private boolean processNodeList(final UserRequest ureq) {
		while (nodeListPos < nodeList.size()) {
			...
			nodeListPos++;
			final Controller ctrl = nextNode.getCourseNode().importNode(getExportDataDir(course), course, false, ureq, getWindowControl());
			if (ctrl != null) {
	 ...
	 * @return File
	 */
	public static File getExportDataDir(final ICourse theCourse) {

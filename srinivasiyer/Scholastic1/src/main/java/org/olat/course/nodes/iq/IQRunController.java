...
public class IQRunController extends BasicController implements GenericEventListener {
	...
	IQRunController(final UserCourseEnvironment userCourseEnv, final ModuleConfiguration moduleConfiguration, final IQSecurityCallback secCallback,
		...
		if (!modConfig.get(IQEditController.CONFIG_KEY_TYPE).equals(AssessmentInstance.QMD_ENTRY_TYPE_ASSESS)) { throw new OLATRuntimeException(
				"IQRunController launched with Test constructor but module configuration not configured as test", null); }
		init(ureq);
	...
	IQRunController(final UserCourseEnvironment userCourseEnv, final ModuleConfiguration moduleConfiguration, final IQSecurityCallback secCallback,
		...
		if (!modConfig.get(IQEditController.CONFIG_KEY_TYPE).equals(AssessmentInstance.QMD_ENTRY_TYPE_SELF)) { throw new OLATRuntimeException(
				"IQRunController launched with Selftest constructor but module configuration not configured as selftest", null); }
		init(ureq);
	...
	IQRunController(final UserCourseEnvironment userCourseEnv, final ModuleConfiguration moduleConfiguration, final IQSecurityCallback secCallback,
		...
		if (!modConfig.get(IQEditController.CONFIG_KEY_TYPE).equals(AssessmentInstance.QMD_ENTRY_TYPE_SURVEY)) { throw new OLATRuntimeException(
				"IQRunController launched with Survey constructor but module configuration not configured as survey", null); }
		init(ureq);
		...
		if (source == startButton && startButton.isEnabled()) {
			...
			if (returnController instanceof IQDisplayController) {
				...
				if (displayController.isReady()) {
					...
					final RepositoryEntry courseRepositoryEntry = RepositoryManager.getInstance().lookupRepositoryEntry(course, true);
	...
	private void exposeUserTestDataToVC(final UserRequest ureq) {
		...
		// user data
		if (!(courseNode instanceof AssessableCourseNode)) { throw new AssertException(
	...
	private void exposeUserSelfTestDataToVC(final UserRequest ureq) {
		...
		if (!(courseNode instanceof SelfAssessableCourseNode)) { throw new AssertException(
	...
	protected void doDispose() {
		...
		if (!type.equals(AssessmentInstance.QMD_ENTRY_TYPE_ASSESS)) { return; }

...
public class AsyncExportManager extends BasicManager {
	...
	public static final AsyncExportManager getInstance() {
		if (INSTANCE == null) { throw new IllegalStateException("AsyncExportManager bean not created via spring. Configuration error!"); }
		return INSTANCE;
	}
	...
	private synchronized void waitForSlot(final Identity identity) {
		...
		while (identitiesOfJobsCurrentlyRunning_.size() > concurrentExportsPerNode_ || identitiesOfJobsCurrentlyRunning_.contains(identity)) {
			try {
				log_.info("waitForSlot: user " + identity.getName() + " wants to archive a course log, but the queue is full. Running count: "
	...
	private synchronized void returnSlot(final Identity identity) {
		...
		log_.info("returnSlot: user " + identity.getName() + " returns a slot. Running count: " + identitiesOfJobsCurrentlyRunning_.size() + ", Total pending jobs: "
				+ waitingCnt_);
		notifyAll();
	}
}

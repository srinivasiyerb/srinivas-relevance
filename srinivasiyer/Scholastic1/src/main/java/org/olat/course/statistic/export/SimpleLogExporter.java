...
public class SimpleLogExporter implements ICourseLogExporter {
	...
	public void exportCourseLog(final File outFile, final String charset, final Long resourceableId, final Date begin, Date end, final boolean resourceAdminAction,
		...
		String query = "select v from org.olat.core.logging.activity.LoggingObject v " + "where v.resourceAdminAction = :resAdminAction " + "AND ( "
				+ "(v.targetResId = :resId) OR " + "(v.parentResId = :resId) OR " + "(v.grandParentResId = :resId) OR " + "(v.greatGrandParentResId = :resId) " + ")";

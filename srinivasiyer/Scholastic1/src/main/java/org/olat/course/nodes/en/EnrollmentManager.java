...
public class EnrollmentManager extends BasicManager {
	...
	public EnrollStatus doEnroll(final Identity identity, final BusinessGroup group, final ENCourseNode enNode, final CoursePropertyManager coursePropertyManager,
			final WindowControl wControl, final Translator trans, final List groupNames, final List areaNames, final CourseGroupManager cgm) {
		final EnrollStatus enrollStatus = new EnrollStatus();
		if (Tracing.isDebugEnabled(this.getClass())) {
			Tracing.logDebug("doEnroll", this.getClass());
		}
		...
		if ((getBusinessGroupWhereEnrolled(identity, groupNames, areaNames, cgm) == null)
			...
			if (Tracing.isDebugEnabled(this.getClass())) {
				Tracing.logDebug("Identity is not enrolled identity=" + identity.getName() + "  group=" + group.getName(), this.getClass());
			...
			CoordinatorManager.getInstance().getCoordinator().getSyncer().doInSync(group, new SyncerExecutor() {
				...
				public void execute() {
					...
					final BusinessGroup reloadedGroup = (BusinessGroup) DBFactory.getInstance().loadObject(group, true);
					if (reloadedGroup.getMaxParticipants() != null && !reloadedGroup.getMaxParticipants().equals("")) {
						...
						Tracing.logInfo("doEnroll - participantsCounter: " + participantsCounter + ", maxParticipants: " + reloadedGroup.getMaxParticipants().intValue(),
								identity.getName(), EnrollmentManager.class);
						if (participantsCounter >= reloadedGroup.getMaxParticipants().intValue()) {
							...
							if (!reloadedGroup.getWaitingListEnabled().booleanValue()) {
								...
								final boolean done = addUserToWaitingList(identity, reloadedGroup, enNode, coursePropertyManager, wControl, trans);
								enrollStatus.setIsInWaitingList(done);
							}
						} else {
							final boolean done = addUserToParticipantList(identity, reloadedGroup, enNode, coursePropertyManager, wControl, trans);
						...
						if (Tracing.isDebugEnabled(this.getClass())) {
							Tracing.logDebug("doEnroll beginTransaction", this.getClass());
						}
						final boolean done = addUserToParticipantList(identity, reloadedGroup, enNode, coursePropertyManager, wControl, trans);
						enrollStatus.setIsEnrolled(done);
						if (Tracing.isDebugEnabled(this.getClass())) {
							Tracing.logDebug("doEnroll committed", this.getClass());
						}
					}
		...
		if (Tracing.isDebugEnabled(this.getClass())) {
			Tracing.logDebug("doEnroll finished", this.getClass());
		}
		return enrollStatus;
	}

	public void doCancelEnrollment(final Identity identity, final BusinessGroup enrolledGroup, final ENCourseNode enNode,
			final CoursePropertyManager coursePropertyManager, final WindowControl wControl, final Translator trans) {
		if (Tracing.isDebugEnabled(this.getClass())) {
			Tracing.logDebug("doCancelEnrollment", this.getClass());
		}
		...
		CoordinatorManager.getInstance().getCoordinator().getSyncer().doInSync(enrolledGroup, new SyncerExecutor() {
			...
			public void execute() {
				...
				final Property lastTime = coursePropertyManager.findCourseNodeProperty(enNode, identity, null, ENCourseNode.PROPERTY_RECENT_ENROLLMENT_DATE);
				if (lastTime != null) {
	...
	public void doCancelEnrollmentInWaitingList(final Identity identity, final BusinessGroup enrolledWaitingListGroup, final ENCourseNode enNode,
		...
		CoordinatorManager.getInstance().getCoordinator().getSyncer().doInSync(enrolledWaitingListGroup, new SyncerExecutor() {
			...
			public void execute() {
				...
				// 2. Remove enrollmentdate property
				// only remove last time date, not firsttime
				final Property lastTime = coursePropertyManager.findCourseNodeProperty(enNode, identity, null, ENCourseNode.PROPERTY_RECENT_WAITINGLIST_DATE);
				if (lastTime != null) {
					coursePropertyManager.deleteProperty(lastTime);
				}
			}
	 ...
	 */
	protected BusinessGroup getBusinessGroupWhereEnrolled(final Identity identity, final List groupNames, final List areaNames, final CourseGroupManager cgm) {
		Iterator iterator = groupNames.iterator();
		// 1. check in groups
		while (iterator.hasNext()) {
			final String groupName = (String) iterator.next();
			final List groups = cgm.getParticipatingLearningGroupsFromAllContexts(identity, groupName);
			if (groups.size() > 0) {
		...
		while (iterator.hasNext()) {
			...
			final List groups = cgm.getParticipatingLearningGroupsInAreaFromAllContexts(identity, areaName);
			if (groups.size() > 0) {
				// Usually it is only possible to be in one group. However,
				// theoretically the
				// admin can put the user in a second enrollment group or the user could
	 ...
	 */
	protected BusinessGroup getBusinessGroupWhereInWaitingList(final Identity identity, final List groupNames, final List areaNames, final CourseGroupManager cgm) {
		final List groups = loadGroupsFromNames(groupNames, areaNames, cgm);
	 ...
	 */
	protected List loadGroupsFromNames(final List groupNames, final List areaNames, final CourseGroupManager cgm) {
		final List groups = new ArrayList();
		// 1. add groups
		Iterator iterator = groupNames.iterator();
		while (iterator.hasNext()) {
			final String groupName = (String) iterator.next();
			final List mygroups = cgm.getLearningGroupsFromAllContexts(groupName);
		...
		while (iterator.hasNext()) {
			final String areaName = (String) iterator.next();
			final List mygroups = cgm.getLearningGroupsInAreaFromAllContexts(areaName);
			for (final Iterator it = mygroups.iterator(); it.hasNext();) {
				final BusinessGroup bg = (BusinessGroup) it.next();
				if (!groups.contains(bg)) {
					groups.add(bg);
	 ...
	 */
	protected boolean hasAnyWaitingList(final List groups) {
		for (final Iterator iter = groups.iterator(); iter.hasNext();) {
	...
	private boolean addUserToWaitingList(final Identity identity, final BusinessGroup group, final ENCourseNode enNode,
			final CoursePropertyManager coursePropertyManager, final WindowControl wControl, final Translator trans) {
		CoordinatorManager.getInstance().getCoordinator().getSyncer().assertAlreadyDoInSyncFor(group);
		// 1. Add user to group, fire events, do loggin etc.

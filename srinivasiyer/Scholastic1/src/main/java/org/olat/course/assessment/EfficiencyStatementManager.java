...
public class EfficiencyStatementManager extends BasicManager implements UserDataDeletable {
	 ...
	 * Constructor
	 */
	private EfficiencyStatementManager(final UserDeletionManager userDeletionManager) {
	 ...
	 * @param userCourseEnv
	 */
	protected void updateUserEfficiencyStatement(final UserCourseEnvironment userCourseEnv) {
		final Long courseResId = userCourseEnv.getCourseEnvironment().getCourseResourceableId();
		final RepositoryEntry re = RepositoryManager.getInstance().lookupRepositoryEntry(OresHelper.createOLATResourceableInstance(CourseModule.class, courseResId),
				false);
	...
	private void updateUserEfficiencyStatement(final UserCourseEnvironment userCourseEnv, final Long repoEntryKey, final ICourse course,
		...
		if (cc.isEfficencyStatementEnabled()) {
			...
			efficiencyStatement.setDisplayableUserInfo(user.getProperty(UserConstants.FIRSTNAME, null) + " " + user.getProperty(UserConstants.LASTNAME, null) + " ("
					+ identity.getName() + ")");
	...
	public Boolean getPassed(final String nodeIdent, final EfficiencyStatement efficiencyStatement) {
		...
		if (assessmentNodes != null) {
			...
			while (iter.hasNext()) {
				final Map<String, Object> nodeData = iter.next();
				if (nodeData.get(AssessmentHelper.KEY_IDENTIFYER).equals(nodeIdent)) { return (Boolean) nodeData.get(AssessmentHelper.KEY_PASSED); }
			}
		}
		return null;
	}
	 ...
	 */
	protected List<EfficiencyStatement> findEfficiencyStatements(final Identity identity) {
		final PropertyManager pm = PropertyManager.getInstance();
		final List<Property> esProperties = pm.listProperties(identity, null, null, PROPERTY_CATEGORY, null);
		final List<EfficiencyStatement> efficiencyStatements = new ArrayList<EfficiencyStatement>();
		final Iterator<Property> iter = esProperties.iterator();
		while (iter.hasNext()) {
			final Property efficiencyProperty = iter.next();
			final EfficiencyStatement efficiencyStatement = (EfficiencyStatement) XStreamHelper.fromXML(efficiencyProperty.getTextValue());
	 ...
	 * @param efficiencyStatement
	 */
	protected void deleteEfficiencyStatement(final Identity identity, final EfficiencyStatement efficiencyStatement) {
		final PropertyManager pm = PropertyManager.getInstance();
	 ...
	 * @param checkForExistingRecord true: check if efficiency statement for this user exist; false: always create new one (be careful with this one!)
	 */

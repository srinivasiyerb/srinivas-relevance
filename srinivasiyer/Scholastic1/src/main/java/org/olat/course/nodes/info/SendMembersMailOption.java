...
public class SendMembersMailOption implements SendMailOption {
	...
	public SendMembersMailOption(final ICourse course, final RepositoryManager rm) {
		this.course = course;
		this.rm = rm;
	}
	...
	public String getOptionTranslatedName(final Locale locale) {
		final Translator translator = Util.createPackageTranslator(SendMembersMailOption.class, locale);
	...
	public List<Identity> getSelectedIdentities() {
		...
		final List<BusinessGroup> learningGroups = cgm.getAllLearningGroupsFromAllContexts();
		for (final BusinessGroup bg : learningGroups) {
			final List<Identity> participants = cgm.getParticipantsFromLearningGroup(bg.getName());
			identities.addAll(participants);
			final List<Identity> coaches = cgm.getCoachesFromLearningGroup(bg.getName());

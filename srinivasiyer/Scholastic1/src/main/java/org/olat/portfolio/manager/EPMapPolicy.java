...
public class EPMapPolicy {
	...
	public Map<String, String> getIdentitiesValue() {
		...
		for (final Identity identity : identities) {
			...
			final String first = identity.getUser().getProperty(UserConstants.FIRSTNAME, null);
			final String last = identity.getUser().getProperty(UserConstants.LASTNAME, null);
	...
	public void addGroup(final BusinessGroup group) {
		...
		for (final BusinessGroup g : groups) {
			if (g.equalsByPersistableKey(group)) { return; }
		}
		groups.add(group);
	}

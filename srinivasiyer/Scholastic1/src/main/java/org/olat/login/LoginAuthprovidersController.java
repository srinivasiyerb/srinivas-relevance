...
public class LoginAuthprovidersController extends MainLayoutBasicController {
	...
	private VelocityContainer initLoginContent(final UserRequest ureq, String provider) {
		...
		final boolean bwo = Settings.isBrowserAjaxBlacklisted(ureq);
		...
		if (authProvider == null) { throw new AssertException("Invalid authentication provider: " + provider); }
	...
	protected void event(final UserRequest ureq, final Controller source, final Event event) {
		if (event == Event.CANCELLED_EVENT) {
			...
			final int loginStatus = AuthHelper.doLogin(identity, BaseSecurityModule.getDefaultAuthProviderIdentifier(), ureq);
			if (loginStatus == AuthHelper.LOGIN_OK) {
				return;
			} else if (loginStatus == AuthHelper.LOGIN_NOTAVAILABLE) {

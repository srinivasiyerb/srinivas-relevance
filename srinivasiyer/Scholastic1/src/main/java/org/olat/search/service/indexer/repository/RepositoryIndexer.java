...
public class RepositoryIndexer implements Indexer {
	 ...
	 */
	private RepositoryIndexer(final RepositoryManager repositoryManager, final SearchModule searchModule) {
	...
	public void doIndex(final SearchResourceContext parentResourceContext, final Object businessObj, final OlatFullIndexer indexWriter) throws IOException,
		...
		if (Tracing.isDebugEnabled(RepositoryIndexer.class)) {
			Tracing.logDebug("RepositoryIndexer repositoryList.size=" + repositoryList.size(), RepositoryIndexer.class);
		}
		...
		while (iter.hasNext()) {
			try {
				...
				if (reloadedRepositoryEntry == null) {
					Tracing.logInfo("doIndex: repositoryEntry was deleted while we were indexing. The deleted repositoryEntry was: " + repositoryEntry,
				...
				if (Tracing.isDebugEnabled(RepositoryIndexer.class)) {
					Tracing.logDebug("Index repositoryEntry=" + repositoryEntry + "  counter=" + counter++ + " with ResourceableId="
							+ repositoryEntry.getOlatResource().getResourceableId(), RepositoryIndexer.class);
				}
				if (!isOnBlacklist(repositoryEntry.getOlatResource().getResourceableId())) {
					...
					if (repositoryEntryIndexer != null) {
						...
						if (Tracing.isDebugEnabled(RepositoryIndexer.class)) {
							Tracing.logDebug("No RepositoryEntryIndexer for " + repositoryEntry.getOlatResource(), RepositoryIndexer.class); // e.g. RepositoryEntry
						}
					}
				} else {
		...
		if (Tracing.isDebugEnabled(RepositoryIndexer.class)) {
			Tracing.logDebug("RepositoryIndexer finished.  counter=" + counter, RepositoryIndexer.class);
		}
	}
	...
	public void setIndexerList(final List indexerList) {
		...
		try {
			for (final Iterator iter = indexerList.iterator(); iter.hasNext();) {
				...
				if (Tracing.isDebugEnabled(RepositoryIndexer.class)) {
					Tracing.logDebug("Adding indexer from configuraton:: ", RepositoryIndexer.class);
				}
			}
	...
	public boolean checkAccess(final ContextEntry contextEntry, final BusinessControl businessControl, final Identity identity, final Roles roles) {
		if (Tracing.isDebugEnabled(RepositoryIndexer.class)) {
			Tracing.logDebug("checkAccess for businessControl=" + businessControl + "  identity=" + identity + "  roles=" + roles, RepositoryIndexer.class);
		}
		...
		if (repositoryEntry != null) {
			...
			if (Tracing.isDebugEnabled(RepositoryIndexer.class)) {
				Tracing.logDebug("isOwner=" + isOwner + "  isAllowedToLaunch=" + isAllowedToLaunch, RepositoryIndexer.class);
			}
			if (isOwner || isAllowedToLaunch) {
				final Indexer repositoryEntryIndexer = RepositoryEntryIndexerFactory.getInstance().getRepositoryEntryIndexer(repositoryEntry);
				if (Tracing.isDebugEnabled(RepositoryIndexer.class)) {
					Tracing.logDebug("repositoryEntryIndexer=" + repositoryEntryIndexer, RepositoryIndexer.class);
				}
				if (repositoryEntryIndexer != null) {

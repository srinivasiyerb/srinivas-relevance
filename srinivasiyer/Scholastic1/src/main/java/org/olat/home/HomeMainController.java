...
public class HomeMainController extends MainLayoutBasicController implements Activateable {
	...
	public HomeMainController(final UserRequest ureq, final WindowControl wControl) {
		...
		if (ureq.getIdentity().getUser().getProperty("emchangeKey", null) != null) {
			...
			final User user = ureq.getIdentity().getUser();
			final String value = user.getProperty("emailDisabled", null);
	...
	private Component createRootComponent(final UserRequest ureq) {
		...
		final StringOutput staticUrl = new StringOutput();
		StaticMediaDispatcher.renderStaticURI(staticUrl, "js/egg.js");
		welcome.put(
				"htmlHeader",
				new HtmlHeaderComponent("rss", null, "<link rel=\"alternate\" type=\"application/rss+xml\" title=\""
	...
	private Controller createController(final String uobject, final UserRequest ureq) {
			...
			if (ae != null) {
				if (anExt.isEnabled()) {
					...
					if (ae instanceof GenericActionExtension && ((GenericActionExtension) ae).getParentTreeNodeIdentifier() != null) {
						...
						if (parentNode == null) { throw new AssertException("could not find parent treeNode: "
								+ ((GenericActionExtension) ae).getParentTreeNodeIdentifier() + ", make sure it gets loaded before child!"); }

...
public class DMZDispatcher implements Dispatcher {
	...
	private boolean rejectRequest(final HttpServletRequest request, final HttpServletResponse response) {
		if (AuthHelper.isRejectDMZRequests()) {
			...
			if (!validBypass) {
				...
				Tracing.logInfo(
						"Rejecting request to DMZDispatcher (AuthHelper.isRejectDMZRequests() is true) to " + rejectUrl + ", sessionId="
								+ request.getRequestedSessionId(), DMZDispatcher.class);
	...
	public void execute(final HttpServletRequest request, final HttpServletResponse response, final String uriPrefix) {
		 ...
		 * Hence it should // be discovered during developing or testing. // String msg = "BAD LINK IN [["+referer+"]]"; Tracing.logWarn(msg, DMZDispatcher.class);
		 * DispatcherAction.sendNotFound(msg, response); return; }
		 */

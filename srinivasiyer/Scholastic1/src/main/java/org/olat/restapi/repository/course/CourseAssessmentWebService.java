...
public class CourseAssessmentWebService {
	...
	public Response getCourseResults(@PathParam("courseId") final Long courseId, @Context final HttpServletRequest request) {
		...
		final List<Identity> courseUsers = loadUsers(course.getCourseEnvironment().getCourseGroupManager());
		int i = 0;
	...
	public Response getCourseResultsOf(@PathParam("courseId") final Long courseId, @PathParam("identityKey") final Long identityKey,
		...
		if (!RestSecurityHelper.isAuthor(httpRequest)) { return Response.serverError().status(Status.UNAUTHORIZED).build(); }

		try {
			final Identity userIdentity = BaseSecurityManager.getInstance().loadIdentityByKey(identityKey, false);
			if (userIdentity == null) { return Response.serverError().status(Status.NOT_FOUND).build(); }
	...
	private void attachAssessableResults(final Long courseResourceableId, final String nodeKey, final Identity requestIdentity, final AssessableResultsVO resultsVO) {
		try {
			...
			if (!(node instanceof AssessableCourseNode)) { throw new IllegalArgumentException("The supplied node key does not refer to an AssessableCourseNode"); }
	...
	private List<Identity> loadUsers(final CourseGroupManager gm) {
		final List<Identity> identites = new ArrayList<Identity>();
		final BaseSecurity securityManager = BaseSecurityManager.getInstance();
		final List<BusinessGroup> groups = gm.getAllLearningGroupsFromAllContexts();

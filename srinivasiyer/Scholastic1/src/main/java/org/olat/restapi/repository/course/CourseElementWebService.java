...
public class CourseElementWebService extends AbstractCourseNodeWebService {
	 ...
	 * @param testResourceableId The test node's id which is retorned in the response of the import test resource
	 * @param position The node's position relative to its sibling nodes (optional)
	 * @param shortTitle The node short title
	 * @param longTitle The node long title
	 * @param objectives The node learning objectives
	...
	public Response attachTest(@PathParam("courseId") final Long courseId, @QueryParam("parentNodeId") final String parentNodeId,
		...
		if (testRepoEntry == null) { return Response.serverError().status(Status.NOT_FOUND).build(); }
	...
	public Response attachWiki(@PathParam("courseId") final Long courseId, @QueryParam("parentNodeId") final String parentNodeId,
		...
		if (wikiRepoEntry == null) { return Response.serverError().status(Status.NOT_FOUND).build(); }
	...
	public Response attachBlog(@PathParam("courseId") final Long courseId, @QueryParam("parentNodeId") final String parentNodeId,
		...
		if (blogRepoEntry == null) { return Response.serverError().status(Status.NOT_FOUND).build(); }
	...
	public Response attachSurvey(@PathParam("courseId") final Long courseId, @QueryParam("parentNodeId") final String parentNodeId,
		...
		if (surveyRepoEntry == null) { return Response.serverError().status(Status.NOT_FOUND).build(); }
	...
	public Response attachTaskFile(@PathParam("courseId") final Long courseId, @PathParam("nodeId") final String nodeId,
		...
		} else if (!(node instanceof TACourseNode)) { return Response.serverError().status(Status.NOT_ACCEPTABLE).build(); }
		if (!isAuthorEditor(course, request)) { return Response.serverError().status(Status.UNAUTHORIZED).build(); }

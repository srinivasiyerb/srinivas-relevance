...
public class CourseGroupWebService {
	...
	public CourseGroupWebService(final OLATResource ores) {
		this.course = ores;
	}
	...
	public Response getGroupList(@Context final HttpServletRequest request) {
		final CourseGroupManager groupManager = PersistingCourseGroupManager.getInstance(course);
		final List<BGContext> groupContexts = groupManager.getLearningGroupContexts();
	...
	public Response putNewGroup(final GroupVO group, @Context final HttpServletRequest request) {
		...
		} else if (course == null) { return Response.serverError().status(Status.NOT_FOUND).build(); }
		...
		if (groupContexts.size() == 1) {
			...
			final BusinessGroup bg = bgm.createAndPersistBusinessGroup(BusinessGroup.TYPE_LEARNINGROUP, ureq.getIdentity(), name, desc, min, max, false, false, context);
	...
	public Response updateGroup(@PathParam("groupKey") final Long groupKey, final GroupVO group, @Context final HttpServletRequest request) {
		if (!RestSecurityHelper.isGroupManager(request)) { return Response.serverError().status(Status.UNAUTHORIZED).build(); }

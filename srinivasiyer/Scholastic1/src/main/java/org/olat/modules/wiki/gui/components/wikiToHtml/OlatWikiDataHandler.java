...
public class OlatWikiDataHandler implements DataHandler {
	...
	public Topic lookupTopic(final String virtualWiki, final String topicName, final boolean deleteOK, final Object transactionObject) throws Exception {
		...
		if (Tracing.isDebugEnabled(OlatWikiDataHandler.class)) {
			...
			try {
				...
				Tracing.logDebug("page name urldecoded and normalized: " + FilterUtil.normalizeWikiLink(URLDecoder.decode(topicName, "utf-8")), OlatWikiDataHandler.class);
				Tracing.logDebug("page name urldecoded normalized and transformed to id: " + wiki.generatePageId(FilterUtil.normalizeWikiLink(decodedName)),
	...
	public boolean exists(final String virtualWiki, final String topic) {
		...
		final Wiki wiki = WikiManager.getInstance().getOrLoadWiki(ores);
		if (topic.startsWith(IMAGE_NAMESPACE) || topic.startsWith(MEDIA_NAMESPACE)) { return wiki.pageExists(topic); }

...
public class WikiToCPExport {
	 ...
	 * @param trans
	 */
	protected WikiToCPExport(final OLATResourceable ores, final Identity ident, final Translator trans) {
		this.ores = ores;
		this.ident = ident;
		this.trans = trans;
	...
	private StringBuilder createJsMappingContent(final Wiki wiki) {
		...
		sb.append("anchor.setAttribute(\"href\", mappings[decodeURI(href.substr(2))]);");
		sb.append("}");
		sb.append("}");
		sb.append("}");
	...
	private StringBuilder createIMSManifest(final Wiki wiki, final Identity ident) {
		...
		if (WikiManager.getInstance().isGroupContextWiki(ores)) {
			final BusinessGroup group = BusinessGroupManagerImpl.getInstance().loadBusinessGroup(ores.getResourceableId(), true);
			name = group.getName();
			sb.append(trans.translate("wiki.exported.from.group", new String[] { name }));
		} else {
			final RepositoryEntry entry = RepositoryManager.getInstance().lookupRepositoryEntry(ores, true);

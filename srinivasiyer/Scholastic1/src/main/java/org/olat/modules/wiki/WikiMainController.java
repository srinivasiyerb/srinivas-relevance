 ...
 * wiki syntax to html is done by @see org.olat.core.gui.components.wikiToHtml.WikiMarkupComponent.
 * <P>
...
public class WikiMainController extends BasicController implements CloneableController {
	...
	WikiMainController(final UserRequest ureq, final WindowControl wControl, final OLATResourceable ores, final WikiSecurityCallback securityCallback,
		...
		if (subsContext != null) {
			...
			final PublisherData data = new PublisherData(OresHelper.calculateTypeName(WikiPage.class), null, businnessPath);
	...
	public void event(final UserRequest ureq, final Controller source, Event event) {
		final Wiki wiki = getWiki();

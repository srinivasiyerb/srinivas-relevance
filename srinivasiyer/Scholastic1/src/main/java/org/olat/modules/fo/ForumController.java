...
public class ForumController extends BasicController implements GenericEventListener {
	...
	public void event(final UserRequest ureq, final Component source, final Event event) {
		if (checkForumChangedEventReceived()) { return; }
	...
	private void showThreadView(final UserRequest ureq, final Message m, String viewMode) {
		...
		final List<Mark> markList = markingService.getMarkManager().getMarks(forumOres, ureq.getIdentity(), markResSubPath);
		...
		final List<MarkResourceStat> statList = markingService.getMarkManager().getStats(forumOres, markResSubPath, ureq.getIdentity());
	...
	private void addMessageToCurrentMessagesAndVC(final UserRequest ureq, final Message m, final VelocityContainer vcContainer, final List<Map<String, Object>> allList,
		...
		String businessPath = currentMark == null ? getWindowControl().getBusinessControl().getAsString() + "[Message:" + m.getKey() + "]" : currentMark
				.getBusinessPath();
		final Controller markCtrl = markingService.getMarkController(ureq, getWindowControl(), currentMark, stat, forumOres, subPath, businessPath);
	...
	private List<Message> prepareListTitles(final List<Message> messages) {
		...
		final MarkingService markingService = (MarkingService) CoreSpringFactory.getBean(MarkingService.class);
		final List<MarkResourceStat> stats = markingService.getMarkManager().getStats(forumOres, null, getIdentity());
	...
	class StickyThreadCellRenderer extends CustomCssCellRenderer {
		...
		protected String getCssClass(final Object val) {
			...
			if (messageWrapper.isSticky()) { return "o_forum_thread_sticky"; }
			return "";
		}

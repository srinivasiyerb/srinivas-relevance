	...
	public void event(final UserRequest ureq, final Controller source, final Event event) {
		if (source == usersForm) {
			if (event == Event.DONE_EVENT) {
				...
				final List<String> isanonymous = new ArrayList<String>();
				final List<String> notfounds = new ArrayList<String>();
				final List<String> alreadyin = new ArrayList<String>();

				final SecurityGroup anonymousSecGroup = securityManager.findSecurityGroupByName(Constants.GROUP_ANONYMOUS);
				...
				for (int i = 0; i < lines.length; i++) {
					...
					if (!username.equals("")) { // skip empty lines
						...
						if (ident == null) { // not found, add to not-found-list
							...
							final boolean inGroup = SyncHelper.containsPersistable(existIdents, ident);
							if (inGroup) {

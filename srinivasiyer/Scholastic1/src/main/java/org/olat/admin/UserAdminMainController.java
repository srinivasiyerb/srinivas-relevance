...
public class UserAdminMainController extends MainLayoutBasicController implements Activateable {
	...
	private Component initComponentFromMenuCommand(final Object uobject, final UserRequest ureq) {
		...
		if (uobject.equals("usearch") || uobject.equals("useradmin")) {
			...
			final SecurityGroup[] secGroup = { BaseSecurityManager.getInstance().findSecurityGroupByName(Constants.GROUP_ADMIN) };
			...
			final SecurityGroup[] secGroup = { BaseSecurityManager.getInstance().findSecurityGroupByName(Constants.GROUP_AUTHORS) };
			contentCtr = new UsermanagerUserSearchController(ureq, getWindowControl(), secGroup, null, null, null, null, Identity.STATUS_VISIBLE_LIMIT, true);
			listenTo(contentCtr);
			return contentCtr.getInitialComponent();
			...
			final UsermanagerUserSearchController myCtr = new UsermanagerUserSearchController(ureq, getWindowControl(), null, permissions, null, null, null,
					Identity.STATUS_VISIBLE_LIMIT, true);
			// now subtract users that are in the author group to get the co-authors
			final SecurityGroup[] secGroup = { BaseSecurityManager.getInstance().findSecurityGroupByName(Constants.GROUP_AUTHORS) };
			final List identitiesFromAuthorGroup = BaseSecurityManager.getInstance()
					.getVisibleIdentitiesByPowerSearch(null, null, true, secGroup, null, null, null, null);
			...
			activatePaneInDetailView = "edit.uroles";
			final SecurityGroup[] secGroup = { BaseSecurityManager.getInstance().findSecurityGroupByName(Constants.GROUP_GROUPMANAGERS) };
			contentCtr = new UsermanagerUserSearchController(ureq, getWindowControl(), secGroup, null, null, null, null, Identity.STATUS_VISIBLE_LIMIT, true);
			listenTo(contentCtr);
			return contentCtr.getInitialComponent();
		} else if (uobject.equals("usermanagergroup")) {
			activatePaneInDetailView = "edit.uroles";
			final SecurityGroup[] secGroup = { BaseSecurityManager.getInstance().findSecurityGroupByName(Constants.GROUP_USERMANAGERS) };
			contentCtr = new UsermanagerUserSearchController(ureq, getWindowControl(), secGroup, null, null, null, null, Identity.STATUS_VISIBLE_LIMIT, true);
			listenTo(contentCtr);
			return contentCtr.getInitialComponent();
		} else if (uobject.equals("usergroup")) {
			activatePaneInDetailView = "edit.uroles";
			final SecurityGroup[] secGroup = { BaseSecurityManager.getInstance().findSecurityGroupByName(Constants.GROUP_OLATUSERS) };
			contentCtr = new UsermanagerUserSearchController(ureq, getWindowControl(), secGroup, null, null, null, null, Identity.STATUS_VISIBLE_LIMIT, true);
			listenTo(contentCtr);
			return contentCtr.getInitialComponent();
		} else if (uobject.equals("anonymousgroup")) {
			activatePaneInDetailView = "edit.uroles";
			final SecurityGroup[] secGroup = { BaseSecurityManager.getInstance().findSecurityGroupByName(Constants.GROUP_ANONYMOUS) };
			contentCtr = new UsermanagerUserSearchController(ureq, getWindowControl(), secGroup, null, null, null, null, Identity.STATUS_VISIBLE_LIMIT, true);
			listenTo(contentCtr);
			return contentCtr.getInitialComponent();

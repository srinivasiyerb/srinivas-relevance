...
public abstract class SingleVMLocker implements Locker, GenericEventListener {
	 ...
	 */
	@Override
	public LockResult acquireLock(final OLATResourceable ores, final Identity identity, final String locksubkey) {
	 ...
	 * aquires a lock
	 * 
	 * @param identity the identity who wishes to obtain the lock
	 * @return the lockresult
	 */
	private LockResult acquireLock(final String key, final Identity identity) {
		...
		synchronized (locks) { // o_clusterOK by:fj, by definition we are in singleVM mode
			...
			if (oldLockEntry == null || identity.getName().equals(oldLockEntry.getOwner().getName())) {
				...
				final LockEntry lockEntry = new LockEntry(key, System.currentTimeMillis(), identity);
	...
	public void event(final Event event) {
		...
		if (!se.isSignOn()) { // it is a "logout" event - we are only interested in logout events
			final String name = se.getIdentityName();
			...
			synchronized (locks) { // o_clusterOK by:fj, by definition we are in singleVM mode
				for (final Iterator<Entry<String, LockEntry>> iter = locks.entrySet().iterator(); iter.hasNext();) {
					...
					final Identity owner = le.getOwner();

...
public class WeeklyCalendarComponentRenderer implements ComponentRenderer {
	...
	public void render(final Renderer renderer, final StringOutput sb, final Component source, final URLBuilder ubu, final Translator translator,
		...
		final List<KalendarEventRenderWrapper> sortedEventsWithinWeek = new ArrayList<KalendarEventRenderWrapper>();
		final List<KalendarEventRenderWrapper> sortedAllDayEventsWithinWeek = new ArrayList<KalendarEventRenderWrapper>();
		boolean hasWriteableCalendar = false;
		for (final Iterator kalendarWrappers = calendarComponent.getKalendars().iterator(); kalendarWrappers.hasNext();) {
			final KalendarRenderWrapper calendarWrapper = (KalendarRenderWrapper) kalendarWrappers.next();
			...
			if (!calendarComponent.isEventAlwaysVisible() && !calendarWrapper.getKalendarConfig().isVis()) {
				continue;
			}
			final boolean readOnlyCalendar = calendarWrapper.getAccess() == KalendarRenderWrapper.ACCESS_READ_ONLY;
			final boolean isImportedCalendar = calendarWrapper.isImported();
			final List events = CalendarUtils.listEventsForPeriod(calendarWrapper.getKalendar(), weekStart, weekEnd);
			for (final Iterator iter = events.iterator(); iter.hasNext();) {
				...
				if (readOnlyCalendar && !isImportedCalendar && event.getClassification() == KalendarEvent.CLASS_PRIVATE) {
					continue;
				}
		...
		// render print view
		final List<KalendarEventRenderWrapper> allEventsOfWeek = new ArrayList<KalendarEventRenderWrapper>(sortedAllDayEventsWithinWeek);
	...
	private void renderEventsListForPrint(final int year, final int weekOfYear, final List<KalendarEventRenderWrapper> sortedEventsWithinWeek, final StringOutput sb,
			final Locale locale) {
		...
		for (int dayCounter = 0; dayCounter < days; dayCounter++) {
			...
			final List<KalendarEventRenderWrapper> sortedEventsOfDay = new ArrayList<KalendarEventRenderWrapper>();
			for (final KalendarEventRenderWrapper eventWrapper : sortedEventsWithinWeek) {
				final KalendarEvent nextEvent = eventWrapper.getEvent();
	...
	private void renderEventsOfDayForPrint(final List<KalendarEventRenderWrapper> sortedEvents, final Date dayStart, final Date dayEnd, final StringOutput sb,
			final Locale locale) {
		if (!sortedEvents.isEmpty()) {
			...
			for (final KalendarEventRenderWrapper event : sortedEvents) {
	...
	private void renderEventForPrint(final KalendarEventRenderWrapper eventWrapper, final Date dayStart, final Date dayEnd, final StringOutput sb, final Locale locale) {
		final KalendarEvent event = eventWrapper.getEvent();
		final boolean hidden = eventWrapper.getCalendarAccess() == KalendarRenderWrapper.ACCESS_READ_ONLY && !eventWrapper.getKalendarRenderWrapper().isImported()
	...
	private void renderAllDayGrid(final int year, final int weekOfYear, final List sortedAllDayEventsForWeek, final StringOutput sb, final URLBuilder ubu,
		...
		for (int day = 1; day <= days; day++) {
			...
			for (final Iterator iter = sortedAllDayEventsForWeek.iterator(); iter.hasNext();) {
				...
				maxDayEventsThisDay++;
				final boolean hideSubject = eventWrapper.getCalendarAccess() == KalendarRenderWrapper.ACCESS_READ_ONLY
	...
	private void renderEventsForDay(final List sortedEvents, final Date dayStart, final int column, final StringOutput sb, final URLBuilder ubu, final Locale locale) {
		...
		for (int eventCounter = 0; eventCounter < sortedEvents.size(); eventCounter++) {
			final KalendarEventRenderWrapper eventWrapper = (KalendarEventRenderWrapper) sortedEvents.get(eventCounter);
			final KalendarEvent event = eventWrapper.getEvent();
			...
			if (!foundSlot) {
				...
				for (final Iterator iter = slotGroup.iterator(); iter.hasNext();) {
					final KalendarEventRenderWrapper rw = (KalendarEventRenderWrapper) iter.next();
			...
			for (final Iterator iter = slotGroup.iterator(); iter.hasNext();) {
				final KalendarEventRenderWrapper rw2 = (KalendarEventRenderWrapper) iter.next();
	...
	private void renderSingleEvent(final KalendarEventRenderWrapper eventWrapper, final StringOutput sb, final URLBuilder ubu, final Locale locale) {
		final KalendarEvent event = eventWrapper.getEvent();
		final boolean hideSubject = eventWrapper.getCalendarAccess() == KalendarRenderWrapper.ACCESS_READ_ONLY && !eventWrapper.getKalendarRenderWrapper().isImported()
				&& event.getClassification() != KalendarEvent.CLASS_PUBLIC;
		String escapedSubject = Formatter.escWithBR(event.getSubject()).toString();
		escapedSubject = escapedSubject.replace('\r', ' ');
	...
	private void renderEventTooltip(final KalendarEventRenderWrapper eventWrapper, final String escapedSubject, final boolean hideSubject, final StringOutput sb,
	...
	private void renderEventLinks(final KalendarEvent event, final StringOutput sb) {

...
public class ClusterSyncer implements Syncer {
	 ...
	 */
	private ClusterSyncer(final PessimisticLockManager pessimisticLockManager) {
	...
	public <T> T doInSync(final OLATResourceable ores, final SyncerCallback<T> callback) {
		...
		long start = 0;
		final boolean isDebug = log.isDebug();
		if (isDebug) {
	...
	public void assertAlreadyDoInSyncFor(final OLATResourceable ores) {
		if (!getData().isEquals(ores) || (getData().getNestedLevel() == 0)) { throw new AssertException("This method must be called from doInSync block with ores="
				+ ores); }
	}
	...
	public void setPessimisticLockManager(final PessimisticLockManager pessimisticLockManager) {
	...
	private class ThreadLocalClusterSyncer {
		...
		protected void decrementNestedLevelCounter() {
			nestedLevelCounter--;
			if (nestedLevelCounter < 0) { throw new AssertException("ClusterSyncer nestedLevelCounter could not be < 0, do not call decrementNestedLevelCounter twice"); }
		}
		...
		protected boolean isEquals(final OLATResourceable ores) {
			if (!this.ores.getResourceableTypeName().equals(ores.getResourceableTypeName())) { return false; }
			if (!this.ores.getResourceableId().equals(ores.getResourceableId())) { return false; }
			return true;
		}
	}

...
public class RSSServlet extends HttpServlet {
	...
	private SyndFeed getPersonalFeed(final String pathInfo) {
		...
		final Identity identity = BaseSecurityManager.getInstance().findIdentityByName(idName);
		if (identity == null) {
			// error - abort
			return null;
		}
		// check if this is a valid authentication
		final Authentication auth = BaseSecurityManager.getInstance().findAuthentication(identity, RSSUtil.RSS_AUTH_PROVIDER);
		if (auth == null) {
		...
		final SyndFeed cachedFeed = null;
		if (cachedFeed != null) {
			// return the cache entry
			return cachedFeed;
		} else {
			// create rss feed for user notifications
			final SyndFeed feed = new PersonalRSSFeed(identity, idToken);
			return feed;
		}
	}
}
...
public class FileChooseCreateEditController extends BasicController {
				...
				if (!isAllowedFileSuffixes(fileName)) {
					...
					if (item != null && (item.canDelete() == VFSConstants.YES)) {
						if (item instanceof OlatRelPathImpl) {
							...
							final MetaInfo meta = MetaInfoFactory.createMetaInfoFor((OlatRelPathImpl) item);
							if (meta != null) {
								meta.deleteAll();
							}
						}
	...
	public void event(final UserRequest ureq, final Component source, final Event event) {
		...
		// delete the chosen file
			...
			final File file = ((LocalFileImpl) vfsItem).getBasefile();
			if (!file.exists()) {
				showError(NLS_ERROR_FILEDOESNOTEXIST);
				return;
			}
	 ...
	 */
	public void setAllowedFileSuffixes(final String[] allowedFileSuffixes) {
	...
	private boolean isAllowedFileSuffixes(String fileName) {
		fileName = fileName.toLowerCase();
		if (allFileSuffixesAllowed) { return true; }

		if (fileName.endsWith(".zip")) { return true; }
		for (int i = 0; i < allowedFileSuffixes.length; i++) {
			if (fileName.endsWith("." + allowedFileSuffixes[i])) { return true; }
		}
		return false;
	}

...
public class BGManagementController extends MainLayoutBasicController implements GenericEventListener {
	...
	public BGManagementController(final UserRequest ureq, final WindowControl wControl, final BGContext bgContext, final BGConfigFlags controllerFlags) {
		...
		if (flags.isEnabled(BGConfigFlags.AREAS)) {
			areaManager = BGAreaManagerImpl.getInstance();
		}
	...
	public void event(final Event event) {
		if (event instanceof BGContextEvent) {
			...
			if (event.getCommand().equals(BusinessGroupModifiedEvent.CONFIGURATION_MODIFIED_EVENT)) {
				...
				final BusinessGroup modifiedGroup = groupManager.loadBusinessGroup(this.currentGroup);
	...
	private void doGroupMessage(final UserRequest ureq) {
		...
		if (flags.isEnabled(BGConfigFlags.RIGHTS)) {
			groupMailToName = translate("rightgroup.message.to");
		} else {
			groupMailToName = translate("group.message.to");
		}
		doContactForm(ureq);
	...
	private void doUserMessage(final UserRequest ureq) {
		...
		sendMessageVC.contextPut(
				...
				getTranslator().translate(
						"user.message",
						new String[] { this.currentIdentity.getUser().getProperty(UserConstants.FIRSTNAME, getLocale()),
	...
	private void doSendMessage(final List identities, final String mailToName, final UserRequest ureq) {
		...
		final ContactList contactList = new ContactList(mailToName);
	...
	private void doGroupDeleteConfirm(final UserRequest ureq) {
		...
		final List<String> deleteableList = groupManager.getDependingDeletablableListFor(currentGroup, ureq.getLocale());
	...
	private void initVC() {
		...
		// User details
		userDetailsVC = new VelocityContainer("userdetails", Util.getPackageVelocityRoot(this.getClass()) + "/userdetails.html", Util.createPackageTranslator(
				HomePageDisplayController.class, getLocale(), getTranslator()), this);
	...
	private void doAddOtherResourcesList(final UserRequest ureq) {
		final List repoTableModelEntries = contextManager.findRepositoryEntriesForBGContext(this.bgContext);
		if (repoTableModelEntries.size() > 1) {
			final Translator resourceTrans = Util.createPackageTranslator(RepositoryTableModel.class, ureq.getLocale(), getTranslator());
	...
	private void doUserDetails(final UserRequest ureq) {
		...
		final Translator babel = UserManager.getInstance().getPropertyHandlerTranslator(userTrans);
		...
		if (flags.isEnabled(BGConfigFlags.GROUP_OWNERS)) {
			final List ownerGroups = groupManager.findBusinessGroupsOwnedBy(bgContext.getGroupType(), this.currentIdentity, bgContext);
		...
		final List participantGroups = groupManager.findBusinessGroupsAttendedBy(bgContext.getGroupType(), this.currentIdentity, bgContext);
	...
	private void doRemoveUserFromParticipatingGroup(final Identity ureqIdentity, final Identity toRemoveIdentity, final String groupKey) {
		...
		final BusinessGroup group = groupManager.loadBusinessGroup(key, true);
	...
	private ContactFormController createContactFormController(final UserRequest ureq) {
		...
		final ContactList ownerCntctLst = new ContactList(businessGroupTranslator.translate("sendtochooser.form.chckbx.owners"));
		final ContactList partipCntctLst = new ContactList(businessGroupTranslator.translate("sendtochooser.form.chckbx.partip"));
		final ContactList waitingListContactList = new ContactList(businessGroupTranslator.translate("sendtochooser.form.chckbx.waitingList"));
		if (flags.isEnabled(BGConfigFlags.GROUP_OWNERS)) {
			if (sendToChooserForm.ownerChecked().equals(BusinessGroupSendToChooserForm.NLS_RADIO_ALL)) {
				final SecurityGroup owners = this.currentGroup.getOwnerGroup();
				...
				if (sendToChooserForm.ownerChecked().equals(BusinessGroupSendToChooserForm.NLS_RADIO_CHOOSE)) {
					final SecurityGroup owners = this.currentGroup.getOwnerGroup();
					final List<Identity> ownerList = scrtMngr.getIdentitiesOfSecurityGroup(owners);
					for (final Identity identity : new ArrayList<Identity>(ownerList)) {
		...
		if (sendToChooserForm != null) {
			if (sendToChooserForm.participantChecked().equals(BusinessGroupSendToChooserForm.NLS_RADIO_ALL)) {
				final SecurityGroup participants = this.currentGroup.getPartipiciantGroup();
				...
				if (sendToChooserForm.participantChecked().equals(BusinessGroupSendToChooserForm.NLS_RADIO_CHOOSE)) {
					final SecurityGroup participants = this.currentGroup.getPartipiciantGroup();
					final List<Identity> participantsList = scrtMngr.getIdentitiesOfSecurityGroup(participants);
					for (final Identity identity : new ArrayList<Identity>(participantsList)) {
						boolean keyIsSelected = false;
		...
		if (sendToChooserForm != null && getIsGMAdminOwner(ureq) && this.currentGroup.getWaitingListEnabled().booleanValue()) {
			if (sendToChooserForm.waitingListChecked().equals(BusinessGroupSendToChooserForm.NLS_RADIO_ALL)) {
				final SecurityGroup waitingList = this.currentGroup.getWaitingGroup();
				...
				if (sendToChooserForm.waitingListChecked().equals(BusinessGroupSendToChooserForm.NLS_RADIO_CHOOSE)) {
					final SecurityGroup waitingList = this.currentGroup.getWaitingGroup();
					final List<Identity> waitingListIdentities = scrtMngr.getIdentitiesOfSecurityGroup(waitingList);
					for (final Identity identity : new ArrayList<Identity>(waitingListIdentities)) {
						boolean keyIsSelected = false;
		...
		final String resourceUrl = JumpInManager.getJumpInUri(this.getWindowControl().getBusinessControl());
		...
		final CollaborationTools collabTools = CollaborationToolsFactory.getInstance().getOrCreateCollaborationTools(this.currentGroup);

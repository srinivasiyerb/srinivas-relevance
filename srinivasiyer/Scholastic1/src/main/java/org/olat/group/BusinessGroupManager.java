...
public interface BusinessGroupManager {
	 ...
	 * @param groupContext the group context or null if no context (e.g. buddy groups)
	 * @param enableWaitinglist
	 * @param enableAutoCloseRanks
	 * @return the created BusinessGroup
	 */
	public BusinessGroup createAndPersistBusinessGroup(String type, Identity identity, String name, String description, Integer minParticipants, Integer maxParticipants,
	 ...
	 * 
	 * @param type Restrict find to this group type or null if not restricted to a specific type
	 * @param identity
	 * @param bgContext Context or null if no context restriction should be applied
	 ...
	 * @param contactLists
	 */
	public void deleteBusinessGroupWithMail(BusinessGroup businessGroupTodelete, WindowControl wControl, UserRequest ureq, Translator trans, List contactLists);
	 ...
	 * @param groupContext The group context or null if group does not belong to a group context (e.g. buddygroups)
	 ...
	 * @param strict true: will throw exception if load failed false: will return null if not found
	 ...
	 * @param targetMin
	 * @param targetMax
	 * @param targetBgContext The group context that the target group will be related to
	 * @param areaLookupMap The area lookup map (the target group will references mapped areas) or null (target group will reference the same areas as the original group)
	 * @param copyAreas
	 ...
	 */
	public BusinessGroup copyBusinessGroup(BusinessGroup sourceBusinessGroup, String targetName, String targetDescription, Integer targetMin, Integer targetMax,
			...
			boolean copyParticipants, boolean copyMemberVisibility, boolean copyWaitingList);
	 ...
	 * @param doOnlyPostAddingStuff true: user has already been added to the security group, do only the other stuff, false: add user to security group first
	 */
	 ...
	 * @param doOnlyPostAddingStuff true: user has already been added to the security group, do only the other stuff, false: add user to security group first
	 */
	public void addParticipantAndFireEvent(Identity ureqIdentity, Identity identity, BusinessGroup group, BGConfigFlags flags, boolean doOnlyPostAddingStuff);
	 ...
	 * @param flags the group configuration flag
	 * @param logger the user activity logger or null if nothing should be logged
	 * @param doOnlyPostRemovingStuff true: user has already been removed from the security group, do only the other stuff, false: remove user from security group first
	 */
	public void removeOwnerAndFireEvent(Identity ureqIdentity, Identity identity, BusinessGroup group, BGConfigFlags flags, boolean doOnlyPostRemovingStuff);

	/**
	 * Remove a user from a group as participant and does all the magic that needs to be done: - remove from secgroup (optional) - remove from jabber roster - fire multi
	 * user event
	 * 
	 * @param wControl
	 * @param ureq the user request of the user who initiates the action
	 ...
	 */
	public BusinessGroupAddResponse moveIdenityFromWaitingListToParticipant(List<Identity> identities, Identity ureqIdentity, BusinessGroup currBusinessGroup,
			BGConfigFlags flags);

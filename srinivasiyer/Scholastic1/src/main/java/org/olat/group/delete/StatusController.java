...
public class StatusController extends BasicController {
	...
	public StatusController(final UserRequest ureq, final WindowControl wControl) {
		...
		myContent.contextPut("header",
				translate("status.delete.email.header", new String[] { Integer.toString(GroupDeletionManager.getInstance().getDeleteEmailDuration()) }));
	...
	protected void updateGroupList() {
		final List l = GroupDeletionManager.getInstance().getGroupsInDeletionProcess(GroupDeletionManager.getInstance().getDeleteEmailDuration());

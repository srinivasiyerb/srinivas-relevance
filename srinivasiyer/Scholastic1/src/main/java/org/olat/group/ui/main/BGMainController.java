...
public class BGMainController extends MainLayoutBasicController implements Activateable {
	...
	/**
	 * deletes this.currBusinessGroup. Checks if user is in owner group, otherwhise does nothing
	 * 
	 * @param ureq
	 */
	private void doBuddyGroupDelete(final UserRequest ureq) {
		...
		final ContactList owners = new ContactList(translate("userlist.owners.title"));
		final List ow = securityManager.getIdentitiesOfSecurityGroup(currBusinessGroup.getOwnerGroup());
		owners.addAllIdentites(ow);
		final ContactList participants = new ContactList(translate("userlist.participants.title"));
		...
		final KalendarRenderWrapper calRenderWrapper = calMan.getGroupCalendar(currBusinessGroup);
	...
	/**
	 * removes user from this.currBusinessGroup's owner and participant group. If no other owner are found the user won't be removed from the owner group
	 * 
	 * @param ureq
	 */
	private void doBuddyGroupLeave(final UserRequest ureq) {
		...
		final BGConfigFlags flags = BGConfigFlags.createBuddyGroupDefaultFlags();
		// 1) remove as owner
		final SecurityGroup owners = currBusinessGroup.getOwnerGroup();
		if (securityManager.isIdentityInSecurityGroup(identity, owners)) {
			final List ownerList = securityManager.getIdentitiesOfSecurityGroup(owners);
			if (ownerList.size() > 1) {
	...
	private BusinessGroup createBuddyGroup(final UserRequest ureq) {
		 ...
		 */
		final BusinessGroup newGroup = bgm.createAndPersistBusinessGroup(BusinessGroup.TYPE_BUDDYGROUP, identity, bgName, bgDesc, bgMin, bgMax, null, null, null);
	 ...
	 * 
	 * @param withLeaveAndDelete config flag: true: leave and delete button are showed, false: not showed
	 * @param ureq
	 */
	...
	private void updateGroupListModelRightgroups() {
		...
		// buddy groups
		// right groups
		final List groups = bgm.findBusinessGroupsAttendedBy(BusinessGroup.TYPE_RIGHTGROUP, identity, null);

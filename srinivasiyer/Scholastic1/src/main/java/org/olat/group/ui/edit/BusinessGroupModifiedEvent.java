...
public class BusinessGroupModifiedEvent extends MultiUserEvent {
	 ...
	 * @param identity the identity for whom the businessGroupsList was built (we only update the list if the identity is affected)
	...
	public boolean updateBusinessGroupList(final List businessGroups, final Identity identity) {
		...
		if (added) {
			...
			final BusinessGroup nGroup = BusinessGroupManagerImpl.getInstance().loadBusinessGroup(modKey, true);
			// if (SyncHelper.)
	 ...
	 * 
	 * @param command The event identifyer, one of CONFIGURATION_MODIFIED_EVENT, IDENTITY_ADDED_EVENT or IDENTITY_REMOVED_EVENT

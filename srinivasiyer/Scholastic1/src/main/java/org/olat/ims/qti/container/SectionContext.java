		...
		if (el_selordering != null) {
			...
			for (final Iterator it_selection = el_selections.iterator(); it_selection.hasNext();) {
				...
				if (el_sourcebankref == null) {
					...
					final Element andornot_selection = (Element) el_selection.selectSingleNode("and_selection|or_selection|not_selection|selection_metadata");
	...
	private void calcFeedBack() {
		...
		for (final Iterator it_oft = el_ofts.iterator(); it_oft.hasNext();) {
			...
			if (fulfilled) {
				...
				final Element el_resolved = (Element) el_section.selectSingleNode(".//sectionfeedback[@ident='" + linkRefId + "']");

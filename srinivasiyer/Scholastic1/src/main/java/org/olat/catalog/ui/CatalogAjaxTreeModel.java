...
public class CatalogAjaxTreeModel extends AjaxTreeModel {
	 ...
	 * @param toBeMovedEntry The entry what should be moved or NULL if no such entry. If not null this entry will not be displayed in the tree
	 * @param ownedEntries List of entries (categories) that are owned by the current user. In category move operations the target category will be checked against this
	 *            list. Can be NULL
	 * @param showLeafs true: show also leafs in the tree; false: show only categories
	 * @param allowDragNdrop true: enable drag&dop of categories and leafs; false: don't use drag&drop
	 */
	...
	public List<AjaxTreeNode> getChildrenFor(final String nodeId) {
		...
		final CatalogEntry entry = cm.loadCatalogEntry(entryKey);
		// load children of node and add them to the list
		final List<CatalogEntry> childEntries = cm.getChildrenOf(entry);

...
public class ChecklistEditCheckpointsController extends FormBasicController {
	...
	public ChecklistEditCheckpointsController(final UserRequest ureq, final WindowControl wControl, final Checklist checklist, final String submitKey,
			final CheckpointComparator checkpointComparator) {
		super(ureq, wControl);
		...
		final int size = checklist.getCheckpoints().size();
	...
	private void addNewFormCheckpoint(final int index, final Checkpoint checkpoint) {
		// add checkpoint title
		final TextElement title = uifactory.addTextElement("title" + counter, null, -1, checkpoint.getTitle(), titleContainer);
		...
		final TextElement description = uifactory.addTextElement("description" + counter, null, -1, checkpoint.getDescription(), titleContainer);
		...
		// add link comment
		final String[] keys = CheckpointMode.getModes();
		final String[] values = new String[keys.length];
		for (int i = 0; i < keys.length; i++) {
			values[i] = translate(keys[i]);
		}
		final SingleSelection mode = uifactory.addDropdownSingleselect("modus" + counter, "form.enableCancelEnroll", titleContainer, keys, values, null);
		...
		final FormLink delButton = new FormLinkImpl("delete" + counter, "delete" + counter, "cl.table.delete", Link.BUTTON_SMALL);
	...
	protected void formInnerEvent(final UserRequest ureq, final FormItem source, final FormEvent event) {
		if (source.getComponent() instanceof Link) {
			if (addButton.equals(source)) {
				...
				final int index = this.checklist.getCheckpoints().size();
				...
				if (checklist.getCheckpoints().size() == 1) {
					...
					final Checkpoint checkpoint = (Checkpoint) ((FormLink) source).getUserObject();
	...
	private void removeFormLink(final Checkpoint checkpoint) {
	...
	protected void initForm(final FormItemContainer fic, final Controller controller, final UserRequest ureq) {
		...
		if (checklist.getCheckpoints().size() == 0) {
			...
			for (int i = 0; i < checklist.getCheckpoints().size(); i++) {
				final Checkpoint checkpoint = checklist.getCheckpointsSorted(checkpointComparator).get(i);

...
public class RepositoryManagerTest extends OlatTestCase {
	...
	public void setup() {
		try {
			...
			Tracing.logError("Error while setting up activeMq or Codepointserver", e, RepositoryManagerTest.class);
		}
	}
	...
	public void testRawRepositoryEntryCreate() {
		try {
			...
			final RepositoryEntry d = new RepositoryEntry();
			d.setOlatResource(r);
	...
	public void testQueryReferencableResourcesLimitType() {
		...
		final List results = rm.queryReferencableResourcesLimitType(id1, new Roles(false, false, false, true, false, false, false), typelist, null, null, null);
	...
	private RepositoryEntry createCourseRepositoryEntry(final int i) {
		...
		// now make a repository entry for this course
		final RepositoryEntry re = RepositoryManager.getInstance().createRepositoryEntryInstance("Florian Gnägi", "Lernen mit OLAT " + i,
	...
	public void testCountByTypeLimitAccess() {
		...
		final int countValueBefore = rm.countByTypeLimitAccess(CourseModule.getCourseTypeName(), RepositoryEntry.ACC_OWNERS_AUTHORS);
		// add 1 entry
	...
	public void testIncrementLaunchCounter() {
		...
		for (int m = 0; m < mainLoop; m++) {
			...
			for (int i = 0; i < loop; i++) {
				...
				final RepositoryEntry repositoryEntryT1 = RepositoryManager.getInstance().lookupRepositoryEntry(keyRepo);
		...
		sleep(20000);
		final RepositoryEntry repositoryEntry2 = RepositoryManager.getInstance().lookupRepositoryEntry(keyRepo);
				...
				try {
					for (int i = 1; i <= loop; i++) {
						...
						if (i % 20 == 0) {
							...
							DBFactory.getInstance().closeSession();
							final RepositoryEntry repositoryEntryT1Reloaded = RepositoryManager.getInstance().lookupRepositoryEntry(keyRepo);
		...
		new Thread(new Runnable() {
			public void run() {
				try {
					...
					final RepositoryEntry repositoryEntryT2 = RepositoryManager.getInstance().lookupRepositoryEntry(keyRepo);
		...
		new Thread(new Runnable() {
			public void run() {
				try {
					...
					final RepositoryEntry repositoryEntryT3 = RepositoryManager.getInstance().lookupRepositoryEntry(keyRepo);

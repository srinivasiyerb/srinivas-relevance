...
public class MarksTest extends OlatTestCase {
	...
	public void testSetMark() {
		for (final Identity ident : identities) {
			final Mark mark = service.getMarkManager().setMark(ores, ident, subPath1, "");
			...
			final boolean marked = service.getMarkManager().isMarked(ores, ident, subPath1);
			assertTrue(marked);
		}
	}
	...
	public void testRemoveMark() {
		...
		final boolean markedAfterRemove = service.getMarkManager().isMarked(ores, ident1, subPath1);
		assertEquals(markedAfterRemove, false);
		final boolean marked = service.getMarkManager().isMarked(ores, ident2, subPath1);
		assertTrue(marked);
	}
	...
	public void testIdentityStats() {
		...
		final List<MarkResourceStat> stats = service.getMarkManager().getStats(ores, subPathList, ident1);
	...
	public void testStats() {
		...
		final List<MarkResourceStat> stats = service.getMarkManager().getStats(ores, subPathList, null);
	 ...
	 * 0; for(Identity ident:loadIdentities) { for(OLATResourceable o:loadOres) { for(String sPath:loadSubPaths) { service.getMarkManager().setMark(o, ident, sPath, "");
	 * if(++count % 20 == 0) { DBFactory.getInstance().intermediateCommit(); } } } } DBFactory.getInstance().intermediateCommit(); long start =
	 * System.currentTimeMillis(); List<MarkResourceStat> stat1s = service.getMarkManager().getStats(loadOres.get(7), null, loadIdentities.get(15));
	 * System.out.println(stat1s.size() + " in (ms): " + (System.currentTimeMillis() - start)); DBFactory.getInstance().intermediateCommit(); start =
	 * System.currentTimeMillis(); List<MarkResourceStat> stat2s = service.getMarkManager().getStats(loadOres.get(18), null, null); System.out.println(stat2s.size() +
	 * " in (ms): " + (System.currentTimeMillis() - start)); DBFactory.getInstance().intermediateCommit(); }
	 */
}
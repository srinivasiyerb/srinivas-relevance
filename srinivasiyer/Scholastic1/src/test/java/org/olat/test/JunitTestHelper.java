...
public class JunitTestHelper {
	...
	public static final Identity createAndPersistIdentityAsUser(final String login) {
		...
		final User user = UserManager.getInstance().createUser("first" + login, "last" + login, login + "@" + maildomain);
		identity = securityManager.createAndPersistIdentityAndUser(login, user, BaseSecurityModule.getDefaultAuthProviderIdentifier(), login, Encoder.encrypt("A6B7C8"));
	...
	public static RepositoryEntry deployDemoCourse() {
		...
		final List<Property> l = propertyManager.findProperties(null, null, null, "_o3_", "deployedCourses");
		if (l.size() > 0) {

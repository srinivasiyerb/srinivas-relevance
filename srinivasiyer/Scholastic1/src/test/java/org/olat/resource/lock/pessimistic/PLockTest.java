...
public class PLockTest extends OlatTestCase {
	...
	public void testReentrantLock() {
		...
		final PLock pc = PessimisticLockManager.getInstance().findOrPersistPLock(asset);
		assertNotNull(pc);
		DBFactory.getInstance().closeSession();

		// test double acquisition within same transaction
		final PLock pc1 = PessimisticLockManager.getInstance().findOrPersistPLock(asset);
		assertNotNull(pc1);
		final PLock pc2 = PessimisticLockManager.getInstance().findOrPersistPLock(asset);
		assertNotNull(pc2);
		DBFactory.getInstance().closeSession();

		// and without explicit transaction boundary.
		final PLock p1 = PessimisticLockManager.getInstance().findOrPersistPLock(asset);
		assertNotNull(p1);
		final PLock p2 = PessimisticLockManager.getInstance().findOrPersistPLock(asset);
		assertNotNull(p2);
	...
	public void testLockWaitTimout() {
		...
		final PLock pc3 = PessimisticLockManager.getInstance().findOrPersistPLock("blibli");
		assertNotNull(pc3);
		DBFactory.getInstance().closeSession();
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					sleep(2500);
					final PLock p3 = PessimisticLockManager.getInstance().findOrPersistPLock("blibli");
					assertNotNull(p3);
				} catch (final Exception e) {
					exceptionHolder.add(e);
				} finally {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					final PLock p2 = PessimisticLockManager.getInstance().findOrPersistPLock("blibli");
					assertNotNull(p2);
					sleep(60000);
	...
	public void testSingleRowLockingSupported() {
		...
		// applied on lock-row-creation cannot support row-level-locking by definition.
		final PLock pc1 = PessimisticLockManager.getInstance().findOrPersistPLock("blabla");
		assertNotNull(pc1);
		final PLock pc2 = PessimisticLockManager.getInstance().findOrPersistPLock("blublu");
		assertNotNull(pc2);
		DBFactory.getInstance().closeSession();
		...
		final PLock p1 = PessimisticLockManager.getInstance().findOrPersistPLock("blabla");
		assertNotNull(p1);

		new Thread(new Runnable() {
			@Override
			public void run() {
				final PLock p2 = PessimisticLockManager.getInstance().findOrPersistPLock("blublu");
				assertNotNull(p2);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					assertNotNull(p2);
					sleep(5000);
					final PLock p3 = PessimisticLockManager.getInstance().findOrPersistPLock("blabla");
					assertNotNull(p3);
				} catch (final Exception e) {
					exceptionHolder.add(e);
				} finally {
	...
	public void testPerf() {
		...
		final int numores = 1;
		final int maxwait = 12; // seconds to wait for completion (upper performance boundary)
		class Collector {
	...
	public void testSync() {
			...
			new Thread(new Runnable() {
				...
				public void run() {
					try {
						...
						//
						final PLock p2 = PessimisticLockManager.getInstance().findOrPersistPLock("befinsert");
						assertNotNull(p2);
		...
		// now count
		DBFactory.getInstance().closeSession();
		final int cnt2 = BaseSecurityManager.getInstance().countIdentitiesOfSecurityGroup(group2);
	...
	void doNoLockingEnrol(final Identity i, final SecurityGroup group) {
		...
		try {
			...
			final int cnt = BaseSecurityManager.getInstance().countIdentitiesOfSecurityGroup(group);

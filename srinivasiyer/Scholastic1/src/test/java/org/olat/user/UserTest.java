...
public class UserTest extends OlatTestCase {
	...
	public void setup() throws Exception {
		...
		if (sm.findIdentityByName("judihui") == null) {
			...
			i1 = sm.createAndPersistIdentityAndUser(u1.getProperty(UserConstants.LASTNAME, new Locale("en")), u1, "OLAT",
					u1.getProperty(UserConstants.LASTNAME, new Locale("en")), "");
		} else {
		...
		if (sm.findIdentityByName("migros") == null) {
			...
			i2 = sm.createAndPersistIdentityAndUser(u2.getProperty(UserConstants.LASTNAME, new Locale("en")), u2, "OLAT",
					u2.getProperty(UserConstants.LASTNAME, new Locale("en")), "");
		} else {
		...
		if (sm.findIdentityByName("salat") == null) {
			...
			i3 = sm.createAndPersistIdentityAndUser(u3.getProperty(UserConstants.LASTNAME, new Locale("en")), u3, "OLAT",
					u3.getProperty(UserConstants.LASTNAME, new Locale("en")), "");
		} else {
	...
	public void testUmCreateUser() throws Exception {
		...
		final UserManager um = UserManager.getInstance();
		final User found = um.findUserByEmail("judihui@id.uzh.ch");
	...
	public void testUmFindUserByKey() throws Exception {
		...
		final User u3test = um.loadUserByKey(u1.getKey());
	...
	public void testUmFindUserByInstitutionalUserIdentifier() throws Exception {
		...
		final List result = BaseSecurityManager.getInstance().getIdentitiesByPowerSearch(null, searchValue, true, null, null, null, null, null, null, null, null);
		assertTrue("must have elements", result != null);
		assertTrue("exactly three elements", result.size() == 3);
		final String instEmailU1 = ((Identity) result.get(0)).getUser().getProperty(UserConstants.INSTITUTIONALEMAIL, null);
		final String instEmailU2 = ((Identity) result.get(1)).getUser().getProperty(UserConstants.INSTITUTIONALEMAIL, null);
		final String instEmailU3 = ((Identity) result.get(2)).getUser().getProperty(UserConstants.INSTITUTIONALEMAIL, null);
	...
	public void testUpdateNewUser() throws Exception {
		final UserManager um1 = UserManager.getInstance();
		final DB db = DBFactory.getInstance();
		final UserImpl u5 = new UserImpl("newuser", "newuser", "new@user.com");
		db.saveObject(u5);
	...
	public void testSetUserProfile() throws Exception {
		...
		final UserManager um = UserManager.getInstance();
		um.updateUser(u1);
		final User u1test = um.loadUserByKey(u1.getKey());
	...
	public void testUmFindCharsetPropertyByIdentity() throws Exception {
		final UserManager um = UserManager.getInstance();
		final User testuser = um.loadUserByKey(u1.getKey());

		final BaseSecurity sm = BaseSecurityManager.getInstance();
		final Identity identity = sm.findIdentityByName(u1.getProperty(UserConstants.LASTNAME, null));

		um.setUserCharset(identity, WebappHelper.getDefaultCharset());

		DBFactory.getInstance().closeSession(); // simulate user clicks
		final String charset = um.getUserCharset(identity);
	...
	public void testOrderByFirstName() {
		...
		final List<Identity> results = db.find(slct.toString());
		final Identity ident1 = (Identity) results.get(0);
		final Identity ident2 = (Identity) results.get(1);
	...
	public void testEquals() {
		final UserManager um = UserManager.getInstance();
		final User user1 = um.findUserByEmail("salat@id.salat.uzh.ch");
		final User user2 = um.findUserByEmail("migros@id.migros.uzh.ch");
		final User user1_2 = um.findUserByEmail("salat@id.salat.uzh.ch");
	...
	public void testEqualsIdentity() {
		final UserManager um = UserManager.getInstance();
		final Identity ident1 = um.findIdentityByEmail("salat@id.salat.uzh.ch");
		final Identity ident2 = um.findIdentityByEmail("migros@id.migros.uzh.ch");
		final Identity ident1_2 = um.findIdentityByEmail("salat@id.salat.uzh.ch");
